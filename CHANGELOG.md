...
### Added
- Claude Sonnet 4.5 (Preview) model support with extended thinking capabilities
- Enhanced test coverage for text extraction and XML parsing
- Git Bash integration for terminal operations
- New terminal-related tools for improved workflow

### Changed
- Updated Bash tool implementation to use Git Bash
- Improved tool parser with better replacement handling

### Fixed
- Terminal race conditions in command execution
- XML parsing errors in tool responses
...
### Added
- Context7 tool for fetching library documentation from popular frameworks
- Read Image tool with WebP optimization (65% compression, 20x token cost reduction)
- Concurrent image processing for 3x faster performance

### Changed
- Improved image processing pipeline with WebP conversion
- Replaced sharp library with jimp + webp-wasm to eliminate native dependencies

### Fixed
- Cross-platform compatibility issues with image processing
- VSIX package size reduced by removing 208MB native binaries
...
### Added
- Pattern Search tool for advanced regex-based code searching
- Fast Editor tool with collapsible UI and batch editing capabilities
- Copy to clipboard functionality in code blocks

### Changed
- Enhanced Fast Editor UI with improved visual hierarchy
- Better code display with syntax highlighting

### Fixed
- Fast Editor approval state handling
- JSON parsing errors in batch operations
...
### Added
- Timer tool for managing long-running operations and background tasks
- Think tool for explicit reasoning and planning steps
- Grep Search tool for fast codebase-wide text searching
- Get Terminal Output tool for reading terminal content
- VSCode API tool for accessing editor state and workspace information

### Changed
- Optimized tool parser with better XML replacement handling
...
### Added
- Multi Replace String tool for batch file editing
- Insert Edit tool for precise code insertions
- Fetch Webpage tool for web content extraction

### Changed
- Enhanced file editor with better diff view
- Improved XML output formatting for all tools

### Fixed
- Multi-replace string crash on Windows paths
- Insert edit tool line number calculation
...
### Added
- Replace String tool for single-file text replacement
- Git Bash tool for terminal operations
- Kill Bash tool for terminating long-running processes
- Read Progress tool for monitoring task status

### Changed
- Enhanced terminal integration with better process management
- Improved error messages for tool failures

### Fixed
- Terminal process cleanup issues
...
### Added
- Multi-agent system with specialized agents (Planner, Tester, Coder)
- Observer Hook for autonomous error correction and guidance
- Spawn Agent tool for delegating subtasks to specialized agents
- Exit Agent tool for completing sub-agent tasks

### Changed
- Redesigned agent architecture with hook-based extensibility
- Improved prompt engineering with ReAct-style reasoning
- Enhanced file editor with rollback capabilities
- Better error diagnostics with hover hints and related intelligence

### Fixed
- File editor indentation issues
...
### Added
- Move, Remove, Rename tools for file operations
- Terminal tool for advanced shell interactions
- Get Errors tool for diagnostic information

### Changed
- Improved file operation safety with confirmation prompts
- Enhanced diagnostic display with better formatting

### Fixed
- File operation race conditions
- Diagnostic handler performance issues
...
### Changed
- Unified provider architecture for better extensibility
...
- mid merge v1.8.0 https://github.com/Microsoft/claude-coder/pull/11
...
**Maintained by**: Vlinder Contributors
**Project Website**: https://vlinders.org/
**Repository**: https://github.com/QuickerStudio/Vlinder
