...
const copyAssetsPlugin = {
	...
	setup(build) {
		build.onEnd(() => {
			...
			const treeSitterSourceDir = path.join(
				__dirname,
				'node_modules',
				'web-tree-sitter'
			);
			...
			fs.copyFileSync(
				...
				path.join(codiconsTargetDir, 'codicon.css')
			);
			fs.copyFileSync(
				path.join(codiconsDir, 'codicon.ttf'),
				path.join(codiconsTargetDir, 'codicon.ttf')
			);
		});
	},
};
	...
	setup(build) {
		build.onEnd(async () => {
			...
			if (!fs.existsSync(rootLibsqlDir)) {
				console.warn(
					`No @libsql folder found at ${rootLibsqlDir}, skipping unpack.`
				);
				return;
			}
			// check if the dist/node_modules/@libsql folder exists if so we skip the unpacking
			if (
				fs.existsSync(path.join(__dirname, 'dist', 'node_modules', '@libsql'))
			) {
				console.warn('libsql binaries already unpacked, skipping.');
				return;
			}

			const distLibsqlDir = path.join(
				__dirname,
				'dist',
				'node_modules',
				'@libsql'
