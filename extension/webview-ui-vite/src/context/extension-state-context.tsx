...
const gitCommitterTypeAtom = atom<'anthropic' | 'user'>('anthropic');
...
const soundEnabledAtom = atom(true);
soundEnabledAtom.debugLabel = 'soundEnabled';
const alertsEnabledAtom = atom(true);
alertsEnabledAtom.debugLabel = 'alertsEnabled';
const timerSoundEnabledAtom = atom(true);
timerSoundEnabledAtom.debugLabel = 'timerSoundEnabled';
...
export const extensionStateAtom = atom((get) => ({
  ...
  soundEnabled: get(soundEnabledAtom),
  alertsEnabled: get(alertsEnabledAtom),
  timerSoundEnabled: get(timerSoundEnabledAtom),
...
const compressionStatusAtom = atom<'idle' | 'compressing' | 'completed'>(
  'idle'
);
    ...
    if (message.type === 'claudeMessage') {
      ...
      setClaudeMessages((currentMessages) => {
        ...
        const index = currentMessages.findIndex(
          (m) => m.ts === message.claudeMessage!.ts
        );
...
export const ExtensionStateProvider: React.FC<{
  ...
  const setAlertsEnabled = useSetAtom(alertsEnabledAtom);
  const setTimerSoundEnabled = useSetAtom(timerSoundEnabledAtom);
  ...
  const setTerminalCompressionThreshold = useSetAtom(
    terminalCompressionThresholdAtom
  );
  ...
  const handleMessage = (event: MessageEvent) => {
    ...
    if (message.type === 'state' && message.state) {
      ...
      setGitCommitterType(message.state.gitCommitterType ?? 'anthropic');
      setTerminalCompressionThreshold(
        message.state.terminalCompressionThreshold
      );
      setAutoSummarize(!!message.state.autoSummarize);
      setSoundEnabled(message.state.soundEnabled ?? true);
      setAlertsEnabled(message.state.alertsEnabled ?? true);
      setTimerSoundEnabled(message.state.timerSoundEnabled ?? true);
