...
export function getSyntaxHighlighterStyleFromTheme(
  themeName: string
): SyntaxHighlighterStyle | undefined {
  const colors = getVSCodeThemeColors();
  ...
  const defaultSyntaxHighlighterTheme = Object.entries(
    defaultSyntaxHighlighterThemes
  ).find(([key]) => key.toLowerCase().startsWith(themeName.toLowerCase()))?.[1];
  if (
    defaultSyntaxHighlighterTheme &&
    defaultSyntaxHighlighterTheme in defaultThemes
  ) {
    return defaultThemes[
      defaultSyntaxHighlighterTheme as keyof typeof defaultThemes
    ];
  }

  const generatedSyntaxHighlighterTheme = Object.entries(
    generatedSyntaxHighlighterThemes
  ).find(([key]) => key.toLowerCase().startsWith(themeName.toLowerCase()))?.[1];
  if (
    generatedSyntaxHighlighterTheme &&
    generatedSyntaxHighlighterTheme in generatedThemes
  ) {
    return generatedThemes[
      generatedSyntaxHighlighterTheme as keyof typeof generatedThemes
    ];
  }
