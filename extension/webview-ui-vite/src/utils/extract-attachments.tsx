import AttachmentsList, {
  FileItem,
  UrlItem,
} from '@/components/chat-row/file-list';
...
export function extractAdditionalContext(
  ...
  const regex =
    /<additional-context(?:\s+[^>]*)?>[\s\S]*?<\/additional-context>/;
...
export const TextWithAttachments = ({ text }: { text?: string }) => {
  ...
  return (
    <div className='flex w-full max-w-[100vw]'>
      {/* 🎯 修复：移除max-w-md限制，让文本容器能够跟随窗口宽度变化 */}
      <span className='w-full break-words' style={{ color: '#66FFDA' }}>
        {mainContent}
      </span>
