...
const PresetsDropdown: React.FC<PresetsDropdownProps> = ({
  onSelectPreset,
}) => {
    ...
    <TooltipProvider>
      <DropdownMenu>
        ...
        <DropdownMenuContent>
          ...
          <DropdownMenuSeparator />
          <DropdownMenuItem
            onClick={() => onSelectPreset('http://localhost:11434/v1')}
            className='text-sm'
          >
            Ollama
          </DropdownMenuItem>
          <DropdownMenuItem
            onClick={() => onSelectPreset('http://127.0.0.1:1234/v1')}
            className='text-sm'
          >
            LMStudio
          </DropdownMenuItem>
...
const ProviderManager: React.FC = () => {
  ...
  const saveSettings = async (settings: ProviderSettings) => {
    try {
      const existingProvider = providersData?.providers?.find(
        (p) => p.providerId === settings.providerId
      );
  ...
  const updateSettings = (
    field: DistributedKeys<ProviderSettings>,
          ...
          <>
            ...
            <div className='space-y-2'>
              ...
              <Input
                ...
                onChange={(e) =>
                  updateSettings('secretAccessKey', e.target.value)
                }
                className='h-8'
              />
            </div>
          ...
          <>
            <div className='space-y-2'>
              ...
              <div className='flex relative items-center'>
                ...
                />
                <PresetsDropdown
                  onSelectPreset={(url) => updateSettings('baseUrl', url)}
                />
              </div>
            ...
            <div className='flex items-center space-x-2'>
              <Switch
                ...
                onCheckedChange={(checked) =>
                  updateSettings('supportImages', checked)
                }
              />
            ...
            <div className='space-y-2'>
              ...
              <Input
                ...
                onChange={(e) =>
                  updateSettings('inputTokensPrice', parseFloat(e.target.value))
                }
                className='h-8'
              />
            </div>
            ...
            <div className='space-y-2'>
              <Label htmlFor='cacheReadsPrice'>
                Cache Reads Price (Optional)
              </Label>
              <Input
                id='cacheReadsPrice'
                type='number'
                ...
                onChange={(e) =>
                  updateSettings('cacheReadsPrice', parseFloat(e.target.value))
                }
                className='h-8'
              />
            </div>
            <div className='space-y-2'>
              <Label htmlFor='cacheWritesPrice'>
                Cache Writes Price (Optional)
              </Label>
              <Input
                id='cacheWritesPrice'
                type='number'
                ...
                onChange={(e) =>
                  updateSettings('cacheWritesPrice', parseFloat(e.target.value))
                }
                className='h-8'
              />
            </div>
  ...
  const currentProvider = providersData?.providers.find(
    (p) => p.providerId === providerSettings?.providerId
  );
          ...
          <div className='space-y-4'>
            ...
            <div className='space-y-4'>
              ...
              <div className='flex gap-2'>
                <Button
                  onClick={() =>
                    providerSettings && saveSettings(providerSettings)
                  }
                  className={currentProvider ? 'flex-1 h-9' : 'w-full h-9'}
                >
                  Save Settings
                </Button>
                {currentProvider && (
      ...
      <Dialog open={showApplyModel} onOpenChange={setShowApplyModel}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Would you like to apply this model as your current model?
            </DialogDescription>
          </DialogHeader>
