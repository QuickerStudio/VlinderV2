...
export const preferencesViewAtom = atom<'select-model' | 'provider-manager'>(
  'select-model'
);
export const forceViewAtom = atom<'select-model' | 'provider-manager'>(
  'select-model'
);
...
export const useRequiredProviderHandler = () => {
  ...
  useEvent('message', (e: MessageEvent) => {
    ...
    if (message.type === 'configureApiRequired') {
      ...
      setPreferencesView('provider-manager');
      setProviderSettings(
        message.providerId ? createDefaultSettings(message.providerId) : null
      );
      setShowSettings(true);
    }
  });
