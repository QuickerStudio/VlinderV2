...
export const ObserverAgentCard = () => {
  ...
  const {
    mutate: customizeObserverPrompt,
    isPending: customizeObserverPromptPending,
  ...
  const {
    ...
    {},
    {
      ...
      refetchOnMount: true,
      refetchOnWindowFocus: true,
      enabled: observerEnabled, // Only run query when observer is enabled
    }
  );
  const { mutate: setObserverEnabled } =
    rpcClient.enableObserverAgent.useMutation({
      onSuccess: () => {
        refetch();
      },
    });
    ...
    <Card>
      ...
      <CardContent className='space-y-4'>
        <CardDescription className='text-xs'>
          An intelligent observer that monitors Vlinder's actions in real-time,
          providing feedback and insights to help optimize performance. The
          observer analyzes patterns, suggests improvements, and helps maintain
          alignment with your goals through continuous evaluation and feedback.
        </CardDescription>
          ...
          <div className='flex flex-col gap-4'>
            <div className='space-y-2'>
              ...
              <div className='text-xs text-muted-foreground mb-2'>
                How often the observer agent should analyze Vlinder's actions.
                Lower values mean more frequent observations but may impact
                performance.
              </div>
              <Slider
            ...
            <div className='space-y-2'>
              ...
              <div className='text-xs text-muted-foreground mb-2'>
                Number of previous messages the observer will review for
                context. More messages provide better context but may increase
                processing time.
              </div>
              <Slider
            ...
            <div className='space-y-2'>
              ...
              <div className='text-xs text-muted-foreground mb-2'>
                The AI model that will analyze Vlinder's actions. Different
                models may offer varying levels of insight and performance.
              </div>
              <ModelSelector
              ...
              >
                ...
                >
                  {modelListData?.models.find(
                    (m) => m.id === observerSettings.modelId
                  )?.name || 'Select Model'}
                  <ChevronDown className='w-4 h-4' />
                </Button>
              </ModelSelector>
              {data.observerSettings?.providerId &&
                data.observerSettings?.providerId !== 'anthropic' &&
            ...
            <div className='space-y-2 mb-4'>
              ...
              <div className='text-xs text-muted-foreground mb-2'>
                Customize the observer's prompt to provide special instructions
                or context for the model.
              </div>
