...
export const ScholarAgentCard = () => {
	...
	const { mutate: customizeScholarPrompt, isPending: customizeScholarPromptPending } =
	...
	const [learningOpportunityDetection, setLearningOpportunityDetection] = useState(true)
	...
	const { mutate: setScholarEnabled } = rpcClient.enableScholarAgent.useMutation({
		onSuccess: () => {
			refetch()
		},
	})
		...
		<Card>
			...
			<CardContent className="space-y-4">
				<CardDescription className="text-xs">
					An intelligent knowledge extraction agent that automatically detects learning opportunities and 
					documents valuable patterns, insights, and best practices from successful development activities. 
					The Scholar Agent helps build a persistent knowledge base for future reference and continuous learning.
				</CardDescription>
				{scholarEnabled && scholarSettings && (
					<div className="flex flex-col gap-4">
						<div className="space-y-3">
							<div className="flex items-center justify-between">
								<Label className="text-xs font-medium">Task Completion Trigger</Label>
								<Switch
							...
							<div className="text-xs text-muted-foreground">
								Automatically trigger Scholar Agent when attempt_completion tool is called. This captures knowledge from completed tasks.
							</div>
						</div>

						<div className="space-y-3">
							<div className="flex items-center justify-between">
								<Label className="text-xs font-medium">Learning Opportunity Detection</Label>
								<Switch
							...
							<div className="text-xs text-muted-foreground">
								Detect learning opportunities from successful problem-solving, pattern discovery, and effective solutions.
							</div>
						</div>

						<div className="space-y-3">
							<div className="flex items-center justify-between">
								<Label className="text-xs font-medium">Explicit Request Trigger</Label>
								<Switch
							...
							<div className="text-xs text-muted-foreground">
								Respond to explicit knowledge requests like "summarize", "remember this", "save knowledge", etc.
							</div>
						</div>
						...
						{learningOpportunityDetection && (
							<div className="space-y-3 pl-4 border-l-2 border-muted">
								...
								<div className="text-xs text-muted-foreground mb-2">
									Keywords that trigger Scholar Hook when detected in conversations. System monitors for these words to automatically start knowledge extraction.
								</div>
								<Textarea
								...
								>
									...
									{savingKeywords ? "Saving..." : keywordsSaved ? "Keywords Saved" : "Save Trigger Keywords"}
								</Button>
							</div>
						)}
						<div className="space-y-2">
							...
							<div className="text-xs text-muted-foreground mb-2">
								Number of previous messages the scholar will review for learning opportunities. 
								More messages provide better context but may increase processing time.
							</div>
							<Slider
						...
						<div className="space-y-2">
							...
							<div className="text-xs text-muted-foreground mb-2">
								The AI model that will analyze development activities and extract knowledge. 
								Different models may offer varying levels of insight and analysis depth.
							</div>
							<ModelSelector
								...
								<Button
									...
									{modelListData?.models.find((m) => m.id === scholarSettings.modelId)?.name ||
										"Select Model"}
						...
						<div className="space-y-2 mb-4">
							...
							<div className="text-xs text-muted-foreground mb-2">
								Customize the scholar's prompt to provide special instructions or context for knowledge
