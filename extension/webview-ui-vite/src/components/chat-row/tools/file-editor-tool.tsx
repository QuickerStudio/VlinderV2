...
import { Edit, Undo2, ExternalLink, AlertTriangle, Check, X } from "lucide-react"
...
export const FileEditorTool: React.FC<FileEditorToolParams & ToolAddons> = memo(
	({
		...
		const handleGitRevert = async () => {
			...
			try {
				const result = await rpcClient.revertToCommit.use({ commitHash: rest.commitHash })
				if (result.success) {
		...
		const renderActionButtons = () => {
			...
			if (extensionState.gitHandlerEnabled && rest.commitHash && approvalState === "approved") {
				buttons.push(
