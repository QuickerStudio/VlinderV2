...
const ChatRowV1: React.FC<ChatRowProps> = ({
    ...
    switch (message.type) {
      case 'say':
        switch (message.say) {
          ...
          case 'hook':
            // Only render Observer Hook, Scholar Hook is now independent
            const hookName = message.hook?.name?.toLowerCase();

            if (hookName === 'observer' || hookName === 'observerhook') {
              return (
                <ObserverBadge
                  state={
                    message.hook?.state === 'pending' ? 'observing' : 'complete'
                  }
            ...
            // Scholar Hook is now handled independently, don't render here
            if (hookName === 'scholar' || hookName === 'scholarhook') {
              return null;
            }

            // Fallback for unknown hook types
            return (
              <div className='inline-flex items-center gap-2 px-3 py-1.5 rounded-md bg-secondary/50 border border-secondary'>
                <span className='text-xs font-mono text-secondary-foreground'>
                  Unknown Hook: {message.hook?.name}
                </span>
              </div>
            );
          ...
          case 'completion_result': {
            ...
            return (
              <>
                <h3
                  className={`flex-line ${message.say === 'error' ? 'text-error' : 'text-success'}`}
                >
                  {icon}
                  {title}
                </h3>
                <div
                  className={
                    message.say === 'error' ? 'text-error' : 'text-success'
                  }
                >
            ...
            return (
              <>
                ...
                >
                  ...
                  <div>
                    Claude won't be able to view the command's output. Please
                    update VSCode (<code>CMD/CTRL + Shift + P</code> → "Update")
                    and make sure you're using a supported shell: zsh, bash,
                    fish, or PowerShell (<code>CMD/CTRL + Shift + P</code> →
                    "Terminal: Select Default Profile").{' '}
                    <a
                      href='https://github.com/Microsoft/claude-coder/wiki/Troubleshooting-terminal-issues'
  ...
  return (
    <section className={'border-none !py-0 !my-0'} data-message-ts={message.ts}>
      <div
        className={cn(
          'my-2 !py-0',
          (message.text?.includes('"tool":"') || message.isSubMessage) &&
            '!hidden',
