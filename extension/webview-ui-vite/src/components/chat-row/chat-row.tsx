...
const ChatRowV1: React.FC<ChatRowProps> = ({ message, isFirst, nextMessage }) => {
	...
	const renderTextContent = () => {
		switch (message.type) {
			case "say":
				switch (message.say) {
					case "unauthorized":
						return <div className="text-muted-foreground text-sm">Authentication is not required</div>
					case "payment_required":
						return <div className="text-muted-foreground text-sm">Payment is not required</div>
	...
	return (
		<section className={"border-none !py-0 !my-0"} data-message-ts={message.ts}>
			...
			{toolContent && (
				<div className={cn("tool-content", textContent ? "mt-4 pt-4" : "mb-2", "my-2")}>
					{toolContent}
				</div>
			)}
		</section>
