import { AlertCircle, ArrowRight, CircleX, X, ChevronDown, ChevronRight, Settings, Gift, Undo2 } from "lucide-react"
...
export const APIRequestMessage: React.FC<{ message: V1ClaudeMessage }> = React.memo(({ message }) => {
	...
	const setMessageHidden = (timestamp: number) => {
		...
		sessionStorage.setItem(hiddenMessagesKey, JSON.stringify([...hiddenMessages]))
	}
	...
	// 检查消息是否已经被隐藏（防止诈尸）
	const [isVisible, setIsVisible] = React.useState(!isMessageHidden(message.ts))
	...
	React.useEffect(() => {
		if (message.isDone && isVisible) { // 只有可见且完成的消息才设置隐藏
	...
	return (
		<>
			...
			<div className={cn(
				...
				{/* Title 作为纯文本 */}
				<span className={cn(
					apiRequestFailedMessage ? "text-destructive" :
					message.isDone ? "text-success" : "text-info"
				)}>
					{title}
				</span>
...
export const UserFeedbackMessage: React.FC<{ message: V1ClaudeMessage }> = React.memo(({ message }) => {
	const { isAllCollapsed, navigateToMessage } = useCollapseState()
	...
	const handleGitRevert = async () => {
		try {
			...
			if (result.success) {
				...
				console.log(`Successfully reverted conversation to message ${message.ts}`)
			} else {
	...
	return (
		<div style={{ display: "flex", alignItems: "start", gap: "8px" }}>
			...
			<div className="flex items-center space-x-2 flex-shrink-0">
				...
				{/* 🎯 Quick Overview模式下显示导航按钮 - 加大尺寸，提高可见性 */}
				{isAllCollapsed && (
					<Button
						size="sm"
						variant="outline"
						className="h-8 w-8 p-0 bg-primary/10 hover:bg-primary/20 border-primary/30 hover:border-primary/50"
						onClick={() => navigateToMessage(message.ts)}
	...
	return (
		<div className="rounded-md p-3 max-w-[360px] mx-auto bg-background/5">
			<div className="flex items-center space-x-3" style={{ color: '#FF63CB' }}>
				...
				<span className="text-sm flex-1 font-semibold">Please configure APIKey!</span>
				<button
					onClick={() => {
