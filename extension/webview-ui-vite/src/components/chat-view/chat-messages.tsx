...
const ChatMessages: React.FC<ChatMessagesProps> = ({ taskId, visibleMessages, syntaxHighlighterStyle }) => {
	const { shouldShowMessage, setMessages, isAllCollapsed } = useCollapseState()
	...
	return (
		<div className="relative overflow-hidden h-full flex flex-col flex-1 ">
			{/* 🎯 Quick Overview border container */}
			<div
				className={cn(
					"h-full chat-content",
					isAllCollapsed && "mx-1 my-5 rounded-lg"
				)}
				style={{
					...(isAllCollapsed && {
						border: '2px solid #66FFDA',
						boxShadow: '0 0 20px #66FFDA40'
					})
				}}
			>
				<Virtuoso
			...
			{!atBottom && userScrolled && (
				<Button
					...
					size="icon"
					variant="secondary"
					className="absolute bottom-8 right-7 transform translate-y-1/2 rounded-full shadow-lg hover:shadow-xl transition-shadow z-50"
