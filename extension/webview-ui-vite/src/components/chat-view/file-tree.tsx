...
export default function EnhancedFileTree({
  ...
  const toggleSelect = (id: string, node: FileNode) => {
    ...
    if (node.type === 'folder') {
      ...
      const allChildrenSelected = allDescendantFiles.every((childId) =>
        newSelectedItems.has(childId)
      );

      if (allChildrenSelected) {
        // Unselect all descendants
        allDescendantFiles.forEach((childId) =>
          newSelectedItems.delete(childId)
        );
      } else {
        // Select all descendants
        if (
          newSelectedItems.size + allDescendantFiles.length <=
          MAX_SELECTED_ITEMS
        ) {
          allDescendantFiles.forEach((childId) =>
            newSelectedItems.add(childId)
          );
        } else {
  ...
  const FileTreeItem = useCallback(
    ({ node }: { node: FileNode }) => {
      ...
      const areAllChildrenSelected =
        firstDepthChildren.length > 0 &&
        firstDepthChildren.every((childId) => value.has(childId));
    ...
    <div className='w-fullp-4 bg-background text-foreground rounded-lg shadow'>
      ...
      {showMaxError && (
        <Alert variant='destructive' className='mb-4'>
          <AlertDescription>
            Maximum selection limit reached. You can select up to{' '}
      ...
      <div
        ref={parentRef}
        className='border border-border rounded overflow-auto h-[400px]'
      >
        <div
          style={{
