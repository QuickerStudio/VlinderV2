...
const InputV2 = forwardRef<HTMLTextAreaElement, InputOpts>(
  (props, forwardedRef) => {
      ...
      if (props.onInsertAt) {
        ...
        setTimeout(() => {
          if (localTextareaRef.current) {
            ...
            localTextareaRef.current.setSelectionRange(
              newText.length,
              newText.length
            );
          }
    ...
    const handleTextareaChange = (
      ...
      if (newAtPositions.length > prevAtPositions.length) {
        // A new "@" was added
        const newAtPosition = newAtPositions.find(
          (pos) => !prevAtPositions.includes(pos)
        );
      ...
      if (showPopover) {
        switch (e.key) {
          ...
          case 'Tab':
            e.preventDefault();
            setFocusedIndex(
              (prevIndex) => (prevIndex + 1) % popoverOptions.length
            );
            break;
          case 'ArrowUp':
            e.preventDefault();
            setFocusedIndex(
              (prevIndex) =>
                (prevIndex - 1 + popoverOptions.length) % popoverOptions.length
            );
            break;
          case 'Enter':
            if (focusedIndex !== -1) {
    ...
    const handleSubmitSelection = () => {
      const newResources: Resource[] = Array.from(selectedItems).map(
        (item) => ({
          id: item,
