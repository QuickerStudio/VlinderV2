		...
		return cleanedMessages.filter((message) => {
			...
			if (
				...
				(message.text === "Let's resume the task from where we left off" ||
					message.text === "Let's continue with the task, from where we left off." ||
					message.text === "Please continue the task from where we left off." ||
					(message.text && message.text.includes("continue the task from where we left off")))
			) {
				return false
			}
	...
	useEffect(() => {
		const handleMessage = (event: MessageEvent) => {
			...
			if (message.type === "contextCompressionComplete") {
				...
				console.error("Context compression failed, blocking further context writes")
			}
		}
					...
					<div className="flex items-center gap-2">
						{isCompressing ? (
							<>
								...
								<span className="text-sm font-medium">Optimizing conversation context...</span>
							</>
						) : (
							<>
								<AlertCircle className="h-4 w-4 text-destructive" />
								<span className="text-sm font-medium text-destructive">Context optimization failed - conversation blocked</span>
							</>
						)}
					</div>
				</div>
