...
import { LightningWindowState, LightningWindowActions } from "@/hooks/use-lightning-window"
...
const MouseCircle: React.FC<MouseCircleProps> = ({ onClick, disabled, isLoading, className }) => {
	...
	const triggerBlink = () => {
		...
		setTimeout(() => {
			if (dotRef.current) {
				dotRef.current.style.transition = 'transform 0.18s cubic-bezier(.4,0,.2,1)'
				dotRef.current.style.transform = 'translate(-50%, -50%) scaleY(1)'
			}
		}, 130)

		// Next blink
		blinkTimerRef.current = setTimeout(triggerBlink, 2000 + Math.random() * 2000)
	}
	...
	const animate = () => {
		...
		if (idleRef.current) {
			...
			if (Math.abs(idleAngleRef.current - idleTargetAngleRef.current) < 0.05 &&
				Date.now() - lastIdleMoveTimeRef.current > 1200 + Math.random() * 800) {
				idleTargetAngleRef.current = Math.random() * Math.PI * 2
				lastIdleMoveTimeRef.current = Date.now()
			}
			idleAngleRef.current += (idleTargetAngleRef.current - idleAngleRef.current) * 0.08
		...
		>
			...
			<div
				...
				style={{
					...
					backgroundColor: isLoading ? 'rgba(248, 113, 113, 0.1)' : 'rgba(0, 200, 83, 0.1)',
					boxShadow: isLoading ? '0 0 12px 2px rgba(248, 113, 113, 0.3)' : '0 0 12px 2px rgba(0, 200, 83, 0.3)'
				}}
			/>
			...
			<div
				...
				style={{
					...
					boxShadow: isLoading ? '0 0 4px 1px rgba(248, 113, 113, 0.5)' : '0 0 4px 1px rgba(34, 211, 238, 0.5)'
				}}
			/>
		</div>
	)
...
const MOOD_PATTERNS = {
	happy: /嘿嘿|哈哈|呵呵|嘻嘻|哇|太好了|真棒|厉害|不错|很好|棒|赞|牛|酷|妙|绝了|完美|优秀|精彩|漂亮|美妙|惊艳|出色|杰出|一流|顶级|超棒|超好|超赞|超牛|超酷|好棒|好赞|好牛|好酷|真好|真棒|真赞|真牛|真酷|太棒了|太赞了|太牛了|太酷了|好极了|棒极了|赞极了|牛极了|酷极了|妙极了|绝极了|hi|Hi|嘿|哈|呀|耶|噢|哟|咦|咯|啦|呢|哦|喔|哇塞|哇哦|哇噻|好耶|太好|好开心|开心|高兴|愉快|兴奋|激动|欣喜|喜悦|快乐|满意|舒服|爽|舒坦|痛快|畅快|😊|😄|😃|🎉|✨|😆|😁|🤗|👍|💪|🥳|😍|🤩|😋|😎|🤤|🙌|👏|🎊|🌟|⭐|💯|🔥|👌|✌️|🤞|🙂|😌|😇|🥰|😘|😗|😙|😚|🤭|🤫|🤔|🤨|😏|😉|😜|😝|🤪|😛|🤓|😺|😸|😹|😻|😽|🙀|😿|😾/g,
	angry: /哎|唉|哼|切|烦|麻烦|讨厌|无语|糟糕|倒霉|郁闷|恼火|气人|可恶|该死|见鬼|要命|头疼|头痛|心烦|心累|累死|烦死|气死|愁死|急死|疯了|崩溃|抓狂|发疯|发狂|受不了|忍不了|不行了|完了|完蛋|糟了|坏了|惨了|死了|毁了|废了|垃圾|破烂|差劲|失望|沮丧|难过|伤心|痛苦|苦恼|忧愁|忧郁|悲伤|悲哀|绝望|无奈|叹气|叹息|唉声叹气|愁眉苦脸|垂头丧气|心灰意冷|心如死灰|欲哭无泪|泪流满面|痛哭流涕|嗯|额|呃|咳|咦|咋|啥|什么鬼|搞什么|怎么回事|什么情况|莫名其妙|不知所云|一头雾水|😠|😡|💢|😤|🙄|😒|😮‍💨|😔|😞|😟|😕|🙁|☹️|😣|😖|😫|😩|🥺|😢|😭|😤|😠|😡|🤬|🤯|😵|😵‍💫|🥴|😪|😴|🤐|🤢|🤮|🤧|🥵|🥶|😰|😨|😧|😦|😮|😯|😲|😳|🥸|😈|👿|💀|☠️|💩|🤡|👹|👺|👻|👽|👾|🤖/g,
}
...
export const Lightning: React.FC<LightningProps> = ({
	...
	useEffect(() => {
		if (wasLoading && !state.isLoading && state.isVisible && textAreaRef.current) {
			setTimeout(() => {
				textAreaRef.current?.focus()
			}, 100)
		}
		...
		>
			...
			<div className="relative w-full h-full flex items-center">
				...
				<div className="absolute top-2 left-2 flex items-center gap-2">
					<div className={`relative ${state.isLoading ? 'w-3 h-3' : 'w-2 h-2'}`}>
						{/* Main dot */}
						<div className={`bg-cyan-400 rounded-full ${state.isLoading ? 'animate-pulse-fast' : 'animate-pulse'} ${state.isLoading ? 'w-3 h-3' : 'w-2 h-2'}`}></div>
				...
				<div className="absolute top-1/2 right-2 transform -translate-y-1/2">
					<div className="relative">
						...
						<MouseCircle
							...
							onClick={state.isLoading ? handleAbortQuestion : handleSendQuestion}
