...
export const ScholarKnowledgeBasePopup: React.FC<
  ScholarKnowledgeBasePopupProps
> = ({ isOpen, onClose, position }) => {
  ...
  const openFile = async (file: ScholarFile) => {
    try {
      const result = await openScholarFileMutation.mutateAsync({
        path: file.path,
      });
      if (result.success) {
  ...
  const deleteFile = async (file: ScholarFile) => {
    try {
      const result = await deleteScholarFileMutation.mutateAsync({
        path: file.path,
      });
      if (result.success) {
            ...
            <div className='flex items-center justify-center py-8'>
              <div className='text-center'>
                ...
                <p className='text-xs text-muted-foreground mt-1'>
                  Scholar will create files as it learns
                </p>
              </div>
            </div>
          ) : (
