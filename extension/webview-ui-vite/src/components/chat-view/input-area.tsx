...
const InputArea: React.FC<InputAreaProps> = ({
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      if (
        message.type === 'compressionStatus' &&
  ...
  const [scholarPopupPosition, setScholarPopupPosition] = useState({
    x: 0,
    y: 0,
  });
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (isScholarPopupOpen) {
        ...
        if (
          !target.closest('[data-scholar-button]') &&
          !target.closest('[data-scholar-popup]')
        ) {
      ...
      <div style={{ position: 'relative' }}>
        ...
        <div
          ...
          style={{
            ...
            borderRadius: '16px',
            border:
              '1px solid var(--vscode-input-border, rgba(255, 255, 255, 0.1))',
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <div>
                      <CircularProgress
                        value={
                          extensionState.currentContextWindow
                            ? Math.round(
                                ((extensionState.currentContextTokens ?? 0) /
                                  extensionState.currentContextWindow) *
                                  100
                              )
                            : 0
                  ...
                  <TooltipContent>
                    <p>
                      Context Window:{' '}
                      {extensionState.currentContextTokens?.toLocaleString() ??
                        0}{' '}
                      /{' '}
                      {extensionState.currentContextWindow?.toLocaleString() ??
                        0}{' '}
                      tokens (
                      {extensionState.currentContextWindow
                        ? Math.round(
