 ...
 * @returns 连接结果或null
 */
function tryBuildConnection(lines: string[], startIndex: number): { statement: string; nextIndex: number } | null {
  let i = startIndex;
  const parts: string[] = [];
  ...
  if (i < lines.length) {
    const connectionMatch = lines[i]?.match(/^(-->|---|-\.-|==>|===|-.->|<-->|<--->|x--x|o--o|->>|-->>|-\)|-\(|<\|--|--\|>|\|\|--|\.\.\.|<\.\.|\.\.>)$/);
    if (connectionMatch) {
 ...
 * 尝试构建简单的连接语句（从连接符开始）
 */
function tryBuildSimpleConnection(lines: string[], startIndex: number): { statement: string; nextIndex: number } | null {
  let i = startIndex;
  const parts: string[] = [];
...
export function formatMermaidCode(code: string): string {
  ...
  while (i < lines.length) {
    ...
    // 检查图表类型声明
    const graphTypeMatch = line.match(/^(graph|flowchart|sequenceDiagram|classDiagram|stateDiagram|stateDiagram-v2|erDiagram|journey|gitgraph|pie|quadrantChart|requirement|mindmap|timeline|zenuml|sankey|block-beta|gantt)$/i);
    ...
    // 检查特殊关键字和指令
    const specialKeywordMatch = line.match(/^(participant|class|classDef|note|title|dateFormat|section|activate|deactivate)$/i);
    if (specialKeywordMatch && i + 1 < lines.length) {
      ...
      if (keyword.toLowerCase() === 'participant' && i + 3 < lines.length) {
        ...
        if (i + 3 < lines.length) {
          ...
          if (forOver.toLowerCase() === 'for' || forOver.toLowerCase() === 'over') {
    ...
    const connectionOnlyMatch = line.match(/^(-->|---|-\.-|==>|===|-.->|<-->|<--->|x--x|o--o|->>|-->>|-\)|-\(|<\|--|--\|>|\|\|--|\.\.\.|<\.\.|\.\.>)$/);
    const shapeOnlyMatch = line.match(getShapePattern());
    const specialKeywordOnlyMatch = line.match(/^(as|for|over|owns|implements|extends)$/i);
    
    if (labelOnlyMatch || connectionOnlyMatch || shapeOnlyMatch || specialKeywordOnlyMatch) {
  ...
  while (j < processedLines.length) {
    ...
    const connectionMatch = line.match(/^(.+)\s+(-->|---|-\.-|==>|===|-.->|<-->|<--->|x--x|o--o)\s+([A-Za-z_][A-Za-z0-9_]*)$/);
    ...
    if (connectionMatch && j + 1 < processedLines.length) {
      ...
      if (shapeMatch) {
        finalProcessedLines.push(`${sourceWithShape} ${connector} ${targetNode}${shapeMatch[1]}`);
        j += 2;
        continue;
      }
    }
  ...
  for (const line of finalProcessedLines) {
    // 检查图表类型声明
    const graphTypeMatch = line.match(/^(graph|flowchart|sequenceDiagram|classDiagram|stateDiagram|stateDiagram-v2|erDiagram|journey|gitgraph|pie|quadrantChart|requirement|mindmap|timeline|zenuml|sankey|block-beta|gantt)(\s+(.+))?$/i);
    ...
    // 检查子图和状态嵌套
    if (line.toLowerCase().startsWith('subgraph') || line.toLowerCase().startsWith('state ')) {
    ...
    // 检查样式定义和类应用
    if (line.toLowerCase().startsWith('classdef') || 
