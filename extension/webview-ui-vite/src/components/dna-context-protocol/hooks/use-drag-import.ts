...
export function useDragImport({ onImport, onError, onInfo }: DragImportOptions) {
	const dragCounterRef = useRef(0)

	// Smart Mermaid code extraction
	const extractMermaidCode = useCallback((content: string, filename: string): string | null => {
		...
		const mermaidTypes = [
			...
			'timeline', 'sankey', 'block', 'architecture', 'c4Context'
		]

		const firstLine = trimmedContent.split('\n')[0].toLowerCase().trim()
		const isPureMermaid = mermaidTypes.some(type =>
		...
		if (matches.length > 0) {
			...
			if (matches.length > 1) {
				onInfo(`Detected ${matches.length} Mermaid code blocks, imported the first one`)
			}
		...
		for (const match of codeMatches) {
			...
			const isMermaidCode = mermaidTypes.some(type =>
				codeFirstLine.startsWith(type) || codeFirstLine.startsWith(type + ':')
			)
		...
		if (filename.toLowerCase().includes('mermaid') || filename.endsWith('.mmd')) {
			return trimmedContent
		}
	...
	const handleDrop = useCallback((e: DragEvent) => {
		...
		if (files.length > 1) {
			onInfo('⚠️ Multiple files detected, only the first file will be imported')
		}
	...
	const bindDragEvents = useCallback((element: HTMLElement) => {
