...
export function useEditHistory(options: UseEditHistoryOptions = {}) {
	...
	// 分析内容变化类型和程度
	const analyzeContentChange = useCallback((oldContent: string, newContent: string) => {
	...
	// 生成描述
	const generateDescription = useCallback((changeType: string, linesChanged: number, charDiff: number) => {
		switch (changeType) {
			case 'initial':
				return '初始内容'
			case 'add':
				return linesChanged > 0 ? `添加 ${linesChanged} 行` : `添加 ${charDiff} 字符`
			case 'delete':
				return linesChanged > 0 ? `删除 ${linesChanged} 行` : `删除 ${charDiff} 字符`
			case 'modify':
	...
	// 添加新的编辑记录
	const addEditRecord = useCallback((content: string, description?: string) => {
	...
	}, [currentIndex, nextHistoryId, maxHistorySize, analyzeContentChange, generateDescription])

	// 立即添加编辑记录
	const addEditRecordImmediate = useCallback((content: string, description?: string) => {
	...
	// 跳转到特定历史记录
	const jumpToHistory = useCallback((recordId: string): string | null => {
	...
	const setAsInitialPoint = useCallback((recordId: string) => {
	...
	// 切换记录的pin状态
	const togglePinRecord = useCallback((recordId: string) => {
		const recordIndex = editHistory.findIndex(record => record.id === recordId)
		...
		if (newIsPinned) {
			// Pin: 移动到顶部
			newHistory = [updatedRecord, ...editHistory.filter(r => r.id !== recordId)]
			...
			// Unpin: 按时间戳重新排序
			const unpinnedHistory = newHistory.filter(r => !(r.isPinned ?? false) || r.id === recordId)
			const pinnedHistory = newHistory.filter(r => (r.isPinned ?? false) && r.id !== recordId)

			// 将unpin的记录按时间戳排序插入到合适位置
			const sortedUnpinned = unpinnedHistory.sort((a, b) =>
			...
			// 重新计算当前索引
			const newCurrentIndex = newHistory.findIndex(r => r.id === (editHistory[currentIndex]?.id))
	...
	// 恢复编辑历史
	const restoreHistory = useCallback((history: EditHistoryRecord[], index: number) => {
		...
		// 安全计算nextHistoryId
		try {
			const maxId = validHistory.length > 0
				? Math.max(...validHistory.map(h => {
		...
		// 安全设置lastContent
		if (safeIndex >= 0 && safeIndex < validHistory.length && validHistory[safeIndex]) {
