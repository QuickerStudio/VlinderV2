...
export function useErrorInterceptor({
  ...
  const shouldFilterError = useCallback(
  ...
  const processError = useCallback(
    (message: string, type: 'error' | 'warning'): string | null => {
      const now = Date.now();
      ...
      stats.errorRate =
        stats.totalErrors / Math.max(1, (now - stats.lastErrorTime) / 1000);
  ...
  const analyzeError = useCallback(
    (message: string): { level: 'error' | 'critical'; suggestion?: string } => {
      const analysis: { level: 'error' | 'critical'; suggestion?: string } = {
        level: 'error',
      ...
      const criticalPatterns = [
        {
          ...
          suggestion:
            'Check syntax, there may be spelling errors or missing symbols',
        },
        {
          pattern: /parse error.*line \d+/i,
          suggestion:
            'Syntax parsing failed, check the code on the specified line',
        },
        {
          pattern: /cannot read prop.*undefined/i,
          suggestion: 'Accessed undefined property, check if object exists',
        },
        {
          pattern: /mermaid.*render.*failed/i,
          suggestion:
            'Diagram rendering failed, check if Mermaid syntax is correct',
        },
        {
          pattern: /invalid.*diagram.*type/i,
          suggestion:
            'Unsupported diagram type, please use correct Mermaid syntax',
        },
        {
          pattern: /flowchart.*syntax/i,
          suggestion:
            'Flowchart syntax error, check node and connection line syntax',
        },
        {
          pattern: /sequence.*participant/i,
          suggestion:
            'Sequence diagram participant definition error, check participant syntax',
        },
      ];
      ...
      const commonErrorSuggestions = [
        {
          pattern: /graph.*TD|LR|TB|RL/i,
          suggestion: 'Flowchart direction should be TD, LR, TB or RL',
        },
        {
          pattern: /sequenceDiagram/i,
          suggestion: 'Sequence diagram syntax: participant A; A->>B: message',
        },
        {
          pattern: /classDiagram/i,
          suggestion: 'Class diagram syntax: class User { +name: string }',
        },
        {
          pattern: /stateDiagram/i,
          suggestion: 'State diagram syntax: [*] --> State1',
        },
        {
          pattern: /erDiagram/i,
          suggestion: 'ER diagram syntax: CUSTOMER ||--o{ ORDER : places',
        },
        {
          pattern: /gitgraph/i,
          suggestion:
            'Git graph syntax: commit; branch feature; checkout feature',
        },
      ];
  ...
  const formatMessage = useCallback(
    (message: string, prefix: string): string => {
