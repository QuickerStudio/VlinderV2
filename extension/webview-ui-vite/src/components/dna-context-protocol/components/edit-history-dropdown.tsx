...
export function EditHistoryDropdown({
	...
	// 分离pinned和unpinned记录
	const pinnedRecords = safeEditHistory.filter(record => record.isPinned ?? false)
					...
					<div className="p-3 border-b border-border">
						...
						{safeEditHistory.length > 0 && (
							<p className="text-xs text-muted-foreground mt-1">
								共 {safeEditHistory.length} 条记录，当前位置: {safeCurrentIndex + 1}
								{pinnedRecords.length > 0 && ` • ${pinnedRecords.length} 条已置顶`}
							</p>
						)}
						...
						{displayRecords.length === 0 ? (
							...
							<div className="p-2">
								{displayRecords.map((record, index) => {
									...
									// 在原始数组中找到当前记录的索引
									const originalIndex = safeEditHistory.findIndex(r => r.id === record.id)
									...
									const getChangeIcon = (changeType?: string) => {
										switch (changeType) {
											case 'add': return 'codicon-add'
											case 'delete': return 'codicon-remove'
											case 'modify': return 'codicon-edit'
											case 'initial': return 'codicon-file'
									...
									const getChangeColor = (changeType?: string) => {
										switch (changeType) {
											...
											case 'initial': return 'text-gray-500'
