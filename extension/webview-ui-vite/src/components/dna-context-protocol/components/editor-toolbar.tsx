...
export function EditorToolbar({
  ...
  return (
    <div className='flex items-center justify-between p-2 border-b bg-muted/50'>
      {!isEditorCollapsed && (
        <span className='text-sm font-medium hidden lg:block'>Code Editor</span>
      )}
      <div className='flex items-center gap-1 lg:gap-2 flex-wrap'>
        ...
        <button
          ...
          className='p-1 hover:bg-accent rounded'
          title={
            isEditorCollapsed ? 'Expand code editor' : 'Collapse code editor'
          }
        >
          <span
            className={`codicon ${isEditorCollapsed ? 'codicon-chevron-left' : 'codicon-chevron-right'}`}
          ></span>
        </button>
      </div>
    </div>
