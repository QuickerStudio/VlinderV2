...
import { CheckCircle2, Clock, Loader2, Trash2, Download } from "lucide-react"
	...
	return (
		...
		<div
			...
			<div className="flex flex-col gap-2 p-4 relative group">
				{/* 顶部行：任务名称 + 时钟按钮 + 导出按钮 + 删除按钮 */}
				<div className="flex justify-between items-center">
					{/* 左侧：任务名称 */}
					<div
						className="text-sm line-clamp-1 whitespace-pre-wrap break-words overflow-wrap-anywhere flex-1 mr-2"
					...
					<div className="flex items-center gap-1">
						...
						<Button
							...
							<span className="sr-only">Show Cache Info</span>  {/* 屏幕阅读器文本 */}
						...
						<Button
							...
							<Download size={16} className="text-foreground" />  {/* 下载图标 */}
						</Button>
						{/* 删除按钮 */}
						<Button
							variant="ghost"
							...
							{isLoading[item.id] ? (
								...
								<Trash2 aria-label="Delete" size={16} className="text-foreground" />  // 正常：垃圾桶图标
							)}
						</Button>
					</div>
				</div>
						...
						<div className="flex justify-between items-start gap-4">
							...
							<div className="flex items-center gap-4">
								...
								<div className="flex items-center gap-2">
									...
									<span className="font-medium uppercase">{formatDate(item.ts)}</span>
								</div>
								...
								{!!item.totalCost && (
									<div className="flex items-center gap-2">
										...
										<span>${item.totalCost?.toFixed(4)}</span>  {/* 保留 4 位小数 */}
									</div>
								)}
							</div>
								...
								<div className="flex items-center gap-2 flex-wrap">
									...
									<span className="flex items-center gap-1">
										<svg
											...
											strokeLinejoin="round"
											className="w-3 h-3">
											<ellipse cx="12" cy="5" rx="9" ry="3"></ellipse>  {/* 数据库图标 */}
