...
const HistoryView = ({ onDone }: HistoryViewProps) => {
  ...
  const { mutate: restoreTaskFromDisk } =
    rpcClient.restoreTaskFromDisk.useMutation({});
  ...
  const taskHistorySearchResults = useMemo(() => {
    ...
    pinnedItems.sort((a, b) => b.ts - a.ts);
    unpinnedItems.sort((a, b) => b.ts - a.ts);
        ...
        <div className='flex flex-wrap gap-2'>
          ...
          <Dialog>
            ...
            <DialogContent>
              ...
              <DialogDescription>
                This action cannot be undone. All history will be permanently
                deleted
              </DialogDescription>
            ...
            return (
              <div key={item.id}>
                ...
                <HistoryItem
                  ...
                  onExport={() =>
                    vscode.postMessage({ type: 'exportTaskWithId', text: item.id })
                  }
                  onPin={() =>
                    vscode.postMessage({ type: 'togglePinTask', text: item.id })
                  }
                />
              </div>
            );
