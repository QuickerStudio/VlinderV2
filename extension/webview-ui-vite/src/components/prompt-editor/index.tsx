...
import {
  currentPromptContentAtom,
  isCurrentPreviewAtom,
} from '../settings-view/tools-tab';
import {
  editorVariable,
  TEMPLATE_PLACEHOLDERS,
  TemplateInfo,
} from 'extension/shared/agent/prompt';
...
export const PromptEditor: React.FC<PromptEditorProps> = () => {
  ...
  useEffect(() => {
    ...
    editorRef.current = monaco.editor.create(monacoContainerRef.current, {
      ...
      theme: theme.kind?.includes('light') ? 'vs' : 'vs-dark',
      placeholder:
        'Enter your prompt template here... or {{ to see placeholders }}',
    ...
    const completionProvider = monaco.languages.registerCompletionItemProvider(
      'promptTemplate',
      {
        ...
        provideCompletionItems: (model, position) => {
          ...
          const suggestions: monaco.languages.CompletionItem[] = Object.keys(
            TEMPLATE_PLACEHOLDERS
          ).map((key) => {
            let insertText = `{${key}}}`;
            let label = `{{${key}}}`;
            ...
            return {
              ...
              documentation: {
                value: `**${key}**\n\n${
                  TEMPLATE_PLACEHOLDERS[
                    key as keyof typeof TEMPLATE_PLACEHOLDERS
                  ].description
                }`,
                isTrusted: true,
              },
  ...
  const handleMessage = useCallback((event: MessageEvent) => {
    promptActions.handleMessage(event.data, {
      ...
      onActiveTemplateUpdated: (templateName) => {
        ...
        setTemplates((prev) =>
          prev.map((t) => ({ ...t, isActive: t.name === templateName }))
        );
      },
    });
  }, []);
                  ...
                  <TooltipContent side='bottom' className='max-w-sm'>
                    <div className='text-sm'>
                      ...
                      <div className='space-y-1 text-xs'>
                        <div>
                          1. Create and edit custom prompt templates with
                          dynamic variables
                        </div>
                        <div>
                          2. Preview templates with real-time syntax
                          highlighting and validation
                        </div>
                        <div>
                          3. Save templates for reuse across different AI tasks
                          and workflows
                        </div>
                        <div>
                          4. Load existing templates and modify them to suit
                          specific needs
                        </div>
                        <div>
                          5. Copy generated prompts to clipboard for use in
                          external applications
                        </div>
                        <div>
                          6. Switch between edit and preview modes for optimal
                          workflow
                        </div>
                        <div>
                          7. Manage multiple template versions with
                          active/viewing status tracking
                        </div>
                      </div>
                    </div>
                  </TooltipContent>
          ...
          <div className='flex-1 relative'>
            {preview && (
              <ScrollArea
                viewProps={{
                  className:
                    'h-full px-4 pt-0 bg-[var(--vscode-editor-background)]',
                }}
              >
                      ...
                      >
                        ...
                        <div className='flex gap-2'>
                          <Button
                            ...
                            onClick={() =>
                              handleSetActive(
                                template.isActive ? null : template.name
                              )
                            }
                          >
