...
import {
	GlobalState,
	GlobalStateManager,
} from '../../providers/state/global-state-manager';
...
const providerToAISDKModel = (
	settings: ApiConstructorOptions,
	modelId: string
): LanguageModelV1 => {
	switch (settings.providerSettings.providerId) {
		...
		case PROVIDER_IDS.OPENAI:
			...
			const reasoningEffort = modelId.includes('o3-mini')
				? {
						reasoningEffort: modelId.includes('high')
							? ('high' as const)
							: ('medium' as const),
					}
				: undefined;
		...
		case PROVIDER_IDS.MOONSHOT:
			...
			return createOpenAI({
				apiKey: settings.providerSettings.apiKey,
				baseURL:
					settings.providerSettings.baseUrl || 'https://api.moonshot.cn/v1',
		...
		case PROVIDER_IDS.OPENAICOMPATIBLE:
			const providerSettings = customProviderSchema.safeParse(
...
export class CustomApiHandler implements ApiHandler {
	...
	>[0]): AsyncIterableIterator<SSEResponse> {
		...
		const convertedMessagesFull = convertedMessages.concat(
			convertToAISDKFormat(messages)
		);
		const currentModel =
			this._options.models.find((m) => m.id === modelId) ?? this._options.model;
		...
		const result = streamText({
			...
			temperature:
				currentModel.id === 'deepseek-reasoner'
					? undefined
					: (tempature ?? 0.1), // deepseek-reasoner doesn't support temperature
			...
			if (part.type === 'finish') {
				...
				if (this._options.providerSettings.providerId === PROVIDER_IDS.OPENAI) {
					const cachedPromptTokens =
						part.providerMetadata?.['openai']?.cachedPromptTokens;
					if (typeof cachedPromptTokens === 'number') {
						...
						cache_creation_input_tokens =
							part.usage.promptTokens - cache_read_input_tokens;
					}
				}
				let inputTokens =
				...
				if (
					this._options.providerSettings.providerId === PROVIDER_IDS.ANTHROPIC
				) {
					// Anthropic has a different way of caching
					part.usage.promptTokens = part.usage.promptTokens ?? 0;
					const cachedCreationTokens =
						part.providerMetadata?.['anthropic']?.cacheCreationInputTokens;
					const cachedPromptTokensRead =
						part.providerMetadata?.['anthropic']?.cacheReadInputTokens;
					if (
						typeof cachedPromptTokensRead === 'number' &&
						typeof cachedCreationTokens === 'number'
					) {
