...
export class CompressToolExecution {
	...
	private compressExecution = async (command: string, output: string, isError = -1): Promise<string> => {
		...
		*Example 1: Unit Test Output*
		...
		FAILED (failures=2)
		- test_profile: AssertionError: Profile validation failed
		...
		**Failed Tests:**
		...
		1. \`test_profile\` - *AssertionError*: Profile validation failed.
		2. \`test_data_retrieval\` - *TimeoutError*: Data retrieval took too long.
		
		**Possible Reasons:**
		
		- The \`test_profile\` failure may be due to invalid data format or validation rules.
		...
		*Example 2: Server Access Log*
		...
		192.168.1.15 - - [10/Oct/2023:13:56:40] "POST /api/profile HTTP/1.1" 500 512
		...
		**Summary:**
		...
		- \`/api/profile\` - Encountered a \`500 Internal Server Error\`.
		- \`/api/data\` - Returned a \`404 Not Found\` error.
		
		**Exceptions:**
		
		- **500 Internal Server Error** on \`/api/profile\` may indicate a server-side issue during the profile update process.
		- **404 Not Found** on \`/api/data\` suggests the requested data endpoint does not exist or has been moved.
		
		**Possible Reasons:**
		
		- The server error on \`/api/profile\` could be due to an unhandled exception in the profile handler.
