...
export function truncateHalfConversation(
	...
	const messagePairsToRemove =
		Math.max(1, Math.floor((messages.length - MIN_MESSAGES_TO_KEEP) / 4)) * 2;
...
export async function smartTruncation(
	...
	const compressedMessages = await compressToolFromMsg(
		beforeCompression,
		args[1],
		args[2]
	);
...
export const estimateTokenCount = (message: Anthropic.MessageParam): number => {
	try {
		...
		const imageCount = message.content.filter((block) =>
			isImageBlock(block)
		).length;
 ...
 */
export const estimateTokenCountFromMessages = (
	messages: Anthropic.Messages.MessageParam[]
): number => {
