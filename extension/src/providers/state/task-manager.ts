...
export class TaskManager {
	...
	async clearTask() {
		const now = new Date();
		const MainAgent = this.provider.getMainAgent();

		this.provider.MainAgent = undefined;

		if (MainAgent) {
			MainAgent.abortTask().catch((err) => {
	...
	async handleNewTask(
		...
		const additionalContextBlocks =
			await formatAttachementsIntoBlocks(attachements);
		await this.provider.initWithTask(
			task + additionalContextBlocks,
			compressedImages
		);
	}
	...
	async handleAskResponse(
		...
		const MainAgent = this.provider.getMainAgent();
		if (!MainAgent) {
			console.error('No MainAgent instance found when handling ask response');
			return;
		}
		...
		try {
			...
			await MainAgent.handleWebviewAskResponse(
				askResponse,
	...
	async renameTask(
		...
		if (params.isCurentTask) {
			currentTaskId = this.provider.getMainAgent()?.getStateManager()
				?.state.taskId!;
		}
		if (!currentTaskId) {
		...
		await this.provider
			.getStateManager()
			.updateTaskHistory(taskData.historyItem);
	...
	async exportCurrentTask() {
		const currentTaskId = this.provider.getMainAgent()?.getStateManager()
			?.state.taskId;
		if (currentTaskId) {
	...
	async showTaskWithId(id: string) {
		if (id !== this.provider.getMainAgent()?.getStateManager().state.taskId) {
	...
	async togglePinTask(id: string) {
		const taskHistory =
			(await this.provider.getStateManager().getState()).taskHistory || [];
		...
		await this.provider
			...
			.updateGlobalState('taskHistory', updatedTaskHistory);
		await this.provider.getWebviewManager().postBaseStateToWebview();
	}

	async restoreTaskFromDisk() {
		const currenTasks =
			await GlobalStateManager.getInstance().getGlobalState('taskHistory');
		const taskDirPath = path.join(
			this.provider.getContext().globalStorageUri.fsPath,
			'tasks'
		);
		...
		const tasksToRestore = allTasks.filter(
			(task) => !currenTasks?.find((t) => t.id === task)
		);
	...
	async clearAllTasks() {
		...
		if (taskDirExists) {
			await fs.rm(taskDirPath, { recursive: true });
		}
	}
	...
	public async getTaskWithId(id: string): Promise<{
		...
		const claudeMessagesFilePath = path.join(
			taskDirPath,
		...
		if (!fileExists) {
			vscode.window.showErrorMessage(
				`Task files not found for task, please try closing and opening vscode again`
			);
			// await this.deleteTaskFromState(id)
	...
	public async migrateAllTasks() {
		const isMigrated = await this.provider
			.getGlobalStateManager()
			.getGlobalState('isMigratedTaskCompleted');
		if (isMigrated) {
			return;
		}
		const taskHistory =
		...
		await this.provider
			.getGlobalStateManager()
			.updateGlobalState('isMigratedTaskCompleted', true);
