...
export class ExtensionStateManager {
	...
	async getState() {
		const [
			...
			this.globalStateManager.getGlobalState('soundEnabled'),
			this.globalStateManager.getGlobalState('alertsEnabled'),
			this.globalStateManager.getGlobalState('timerSoundEnabled'),
		...
		const currentTaskId = this.context.getMainAgent()?.getStateManager()
			?.state.taskId;
		const currentClaudeMessage = this.context.getMainAgent()?.getStateManager()
		...
		return {
			...
			currentTaskId,
			alwaysAllowReadOnly:
				alwaysAllowReadOnly === undefined || alwaysAllowReadOnly === null
					? true
					: alwaysAllowReadOnly,
			...
			timerSoundEnabled: timerSoundEnabled ?? true,
	...
	async setAutoCloseTerminal(value: boolean) {
		...
		return this.globalStateManager.updateGlobalState(
			'autoCloseTerminal',
			value
		);
	}
	...
	async updateTaskHistory(
		...
		const history =
			(await this.globalStateManager.getGlobalState('taskHistory')) ?? [];
