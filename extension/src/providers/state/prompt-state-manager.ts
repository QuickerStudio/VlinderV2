...
export class PromptStateManager {
	...
	private async loadState(): Promise<void> {
		...
		this.state.activePromptName =
			GlobalStateManager.getInstance().getGlobalState('activePromptName') ||
			this.DEFAULT_PROMPT_NAME;
	...
	public async getActivePromptContent(): Promise<string> {
		try {
			...
			console.error(
				'Failed to load active prompt, falling back to default:',
				error
			);
			return this.state.defaultPromptContent;
		}
