...
export type GlobalState = {
	...
	gitCommitterType: 'anthropic' | 'user' | undefined;
	...
	timerSoundEnabled: boolean | undefined;
	...
	scholarSettings:
		| {
				/**
				 * The model ID to use for the scholar
				 */
				modelId: string;
				/**
				 * The provider ID that is associated with the model
				...
				providerId: ProviderId;
				/**
				 * The number of last messages to pull to the scholar for knowledge extraction
				 */
				scholarPullMessages: number;
				/**
				 * The number of requests to make before triggering the scholar
				 */
				scholarEveryXRequests: number;
				/**
				 * Custom prompt to use for the scholar
				 */
				scholarPrompt?: string;
				/**
				 * Learning keywords that trigger Scholar Hook when detected in conversations
				 */
...
export class GlobalStateManager {
	...
	public static getInstance(
		...
		if (!GlobalStateManager.instance) {
			if (!context) {
				throw new Error(
					'Context must be provided when creating the GlobalStateManager instance'
				);
			}
	...
	async updatePartialGlobalState<K extends keyof GlobalState>(
		key: K,
	...
	private getKeyDefaultValue<K extends keyof GlobalState>(
		key: K
	): GlobalState[K] {
		if (key in defaults) {
