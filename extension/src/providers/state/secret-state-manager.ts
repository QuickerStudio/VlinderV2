...
export class SecretStateManager {
	...
	public static getInstance(
		...
		if (!SecretStateManager.instance) {
			if (!context) {
				throw new Error(
					'Context must be provided when creating the SecretStateManager instance'
				);
			}
	...
	async updateSecretState<K extends keyof SecretState>(
		key: K,
		value: SecretState[K]
	): Promise<void> {
	...
	async upsertProviderSetting(setting: ProviderSettings): Promise<void> {
		...
		const existingIndex = currentSettings.findIndex(
			(p) => p.providerId === setting.providerId
		);
