...
export class WebviewManager {
	...
	async postClaudeMessagesToWebview(msgs?: ClaudeMessage[] | null) {
		const claudeMessages = this.provider.getMainAgent()?.getStateManager().state.claudeMessages ?? []
	...
	private getHtmlContent(webview: vscode.Webview): string {
		...
		const csp = [
			`default-src 'none';`,
			`script-src 'unsafe-eval' https://* vscode-webview: ${isProd ? `'nonce-${nonce}'` : `http://${localServerUrl} http://0.0.0.0:${localPort} 'unsafe-inline'`
			}`,
			...
			`connect-src https://* vscode-webview: ${isProd
				? ``
				...
				switch (message.type) {
					...
					case "openKnowledgeBase":
						try {
							...
							await vscode.commands.executeCommand("revealInExplorer", knowledgeBaseUri)

							vscode.window.showInformationMessage(`Scholar Knowledge Base opened: ${knowledgeBasePath}`)
						} catch (error) {
							console.error("Error opening knowledge base:", error)
							vscode.window.showErrorMessage("Failed to open Scholar Knowledge Base")
						}
						break
	...
	private async handleCompressContext(message: any): Promise<void> {
		try {
			...
			const { truncateHalfConversation } = await import("../../utils/context-managment/index")
			...
			const taskInfo = currentTask ? ` for task: ${currentTask.title || currentTask.id}` : ''
