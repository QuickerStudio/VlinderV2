...
export class PromptManager {
	...
	private getPromptEditorHtmlContent(webview: vscode.Webview): string {
		...
		let scriptUri;
		const isProd =
			this.webviewManager.provider.getContext().extensionMode ===
		...
		if (isProd) {
			scriptUri = getUri(
				...
				['webview-ui-vite', 'build', 'assets', 'prompt-editor.js']
			);
		} else {
		...
		// 	"App.css",
		// ])
		const stylesUri2 = getUri(
			webview,
			this.webviewManager.provider.getContext().extensionUri,
			['webview-ui-vite', 'build', 'assets', 'index.css']
		);
		const stylesUri = getUri(
			webview,
			this.webviewManager.provider.getContext().extensionUri,
			['webview-ui-vite', 'build', 'assets', 'prompt-editor.css']
		);
	...
	private async createPromptEditorPanel() {
		...
		this.promptEditorPanel.webview.html = this.getPromptEditorHtmlContent(
			this.promptEditorPanel.webview
		);
	...
	private async showPromptEditor() {
		...
		await new Promise<void>((resolve) => {
			const disposable = this.promptEditorPanel!.webview.onDidReceiveMessage(
				(msg) => {
		...
		const content =
			(await promptManager.getActivePromptContent()) ??
			promptManager.getDefaultPromptContent();
		const disabledTools =
			(await GlobalStateManager.getInstance().getGlobalState(
				'disabledTools'
			)) ?? [];
		await Promise.all([
	 ...
	 */
	private async savePromptTemplate(
		templateName: string,
		content: string
	): Promise<void> {
		try {
	...
	private async loadPromptTemplate(templateName: string): Promise<void> {
		try {
			...
			if (error instanceof Error) {
				...
				vscode.window.showErrorMessage(
					'Failed to load template: Unknown error'
				);
			}
		}
	}
	...
	private async setActivePrompt(templateName: string | null): Promise<void> {
		try {
			...
			if (error instanceof Error) {
				vscode.window.showErrorMessage(
					`Failed to set active prompt: ${error.message}`
				);
			} else {
				vscode.window.showErrorMessage(
					'Failed to set active prompt: Unknown error'
				);
			}
		}
	}
	...
	private async deleteTemplate(templateName: string): Promise<void> {
		try {
			...
			if (error instanceof Error) {
				...
				vscode.window.showErrorMessage(
					'Failed to delete template: Unknown error'
				);
			}
		}
	}
		...
		switch (message.type) {
			...
			case 'disableTool':
				const currentDisabledTools =
					GlobalStateManager.getInstance().getGlobalState('disabledTools') ??
					[];
				...
				// Also send to main webview
				promises.push(
					this.webviewManager.postMessageToWebview({
						type: 'disabledTools',
						tools: Array.from(newDisabledTools),
					})
				);
