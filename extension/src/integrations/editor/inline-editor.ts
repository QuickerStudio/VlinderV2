...
export class InlineEditHandler {
	...
	private async openDiffEditor(
		...
		this.originalUri = vscode.Uri.parse(
	...
	public async applyStreamContent(
		id: string,
		searchContent: string,
		content: string
	): Promise<void> {
	...
	public async applyFinalContent(
		id: string,
		searchContent: string,
		...
		if (!this.isOpen()) {
			this.logger(
				`Editor not open, queueing final operation for id ${id}`,
				'debug'
			);
			return;
		}
	...
	private validateDocumentState(): asserts this is {
	...
	private async updateFileContent(): Promise<void> {
		...
		const blocksInInsertionOrder = [
			...this.currentDocumentState.editBlocks.entries(),
		]
			.sort((a, b) => {
				const indexA =
					this.currentDocumentState.editBlocksInsertionIndex.get(a[0]) ?? -1;
				const indexB =
					this.currentDocumentState.editBlocksInsertionIndex.get(b[0]) ?? -1;
		...
		for (const block of blocksInInsertionOrder) {
			...
			const matchResult = findAndReplace(
				newContent,
				block.searchContent,
			...
			if (matchResult.success && matchResult.newContent) {
				...
				const insertionIndex =
					this.currentDocumentState.editBlocksInsertionIndex.get(block.id) ??
					-1;
		...
		if (
			...
			latestAppliedBlockIndex !== -1 &&
			latestAppliedBlockLineEnd !== undefined
		) {
	...
	private async scrollToLine(line: number): Promise<void> {
		...
		const validLine = Math.max(
			0,
			Math.min(line, editor.document.lineCount - 1)
		);
		const range = new vscode.Range(
			validLine,
			0,
			validLine,
			editor.document.lineAt(validLine).text.length
		);
		editor.revealRange(
			range,
	...
	public async saveChanges(): Promise<{
		...
		for (const result of results) {
			if (
				result.wasApplied &&
				typeof result.lineStart === 'number' &&
				typeof result.lineEnd === 'number'
			) {
		...
		const normalizedfinalContentBeforeSave =
			finalContentBeforeSave.replace(/\r\n|\n/g, '\n').trimEnd() + '\n';
		const normalizedStreamedContent =
			finalStreamedContent.replace(/\r\n|\n/g, '\n').trimEnd() + '\n';
	...
	public async forceFinalize(
		...
		await this.updateFileContent();
		const failedBlocks =
			this.currentDocumentState.lastUpdateResults?.filter(
				(r) => !r.wasApplied
			) || [];
		...
		if (isAnyFailed) {
			this.logger(
				`${failedBlocks.length} blocks out of ${blocks.length} failed to apply`,
				'warn'
			);
