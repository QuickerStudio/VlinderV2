...
export class DiffViewProvider {
							...
							if (this.checkScrollPosition()) {
								...
								if (
									...
									now - this.lastUserInteraction <
										DiffViewProvider.USER_INTERACTION_TIMEOUT
								) {
									this.isAutoScrollEnabled = false;
								}
							}
	...
	public async openDiffEditor(
		...
		if (
			editor &&
			editor.document.uri.toString() === this.modifiedUri.toString()
		) {
	...
	public async update(
		...
		await this.updateQueue.add(
			async () => await this.applyLineByLineUpdate(accumulatedContent)
		);
	}
	...
	private async applyLineByLineUpdate(content: string): Promise<void> {
		if (!this.diffEditor || !this.modifiedUri) {
			this.logger(
				'<applyLineByLineUpdate>: Diff editor not initialized',
				'error'
			);
			return;
		}
		if (this.isFinalReached) {
			this.logger(
				'<applyLineByLineUpdate>: Final update already reached',
				'warn'
			);
			return;
		}
		...
		if (this.currentDocument.length === 0) {
			...
			this.currentDocument = currentContent
		...
		const updatedContent = [
			...this.currentDocument.slice(
				0,
				Math.max(newLines.length, this.currentDocument.length)
			),
		].join('\n');
	...
	private async scrollToModifiedLine(): Promise<void> {
		...
		const range = new vscode.Range(
			...
			0,
			line,
			this.diffEditor.document.lineAt(line).text.length
		);
	...
	private checkScrollPosition(): boolean {
		...
		return (
			this.lastModifiedLine >=
				firstVisibleLine - DiffViewProvider.SCROLL_THRESHOLD &&
			this.lastModifiedLine <=
				lastVisibleLine + DiffViewProvider.SCROLL_THRESHOLD
		);
	}
	...
	private async createPrettyPatch(
		filename: string,
		oldStr: string,
		newStr: string
	): Promise<string> {
