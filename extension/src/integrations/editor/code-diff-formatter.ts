export class CodeDiffFormatter {
	...
	public generateTruncatedDiff(
		...
		const structuralLineNumbers = this.findStructuralLines(
			newLines,
			options.language
		);
		...
		const structuralBlocks = this.findStructuralBlocks(
			newLines,
		...
		const combinedRegions = this.mergeOverlappingRegions([
			...structuralRegions,
			...changeRegions,
		]);
	...
	private findChangedLines(
		...
		for (let i = 0; i < Math.max(originalLines.length, newLines.length); i++) {
			if (
				...
				i >= newLines.length ||
				originalLines[i].trim() !== newLines[i].trim()
			) {
				changedLines.add(i);
			}
		}
	...
	private findStructuralLines(
		...
		const patterns =
			languagePatterns[language.toLowerCase()] || languagePatterns.typescript;
	 ...
	 */
	private isLineImportant(
		lineNumber: number,
		importantLines: Set<number>,
		contextLines: number
	): boolean {
		// Check the line itself and surrounding lines within context range
		for (
			let i = lineNumber - contextLines;
			i <= lineNumber + contextLines;
			i++
		) {
	...
	private buildOutput(
		...
		for (const region of regions) {
			...
			if (lastEnd !== -1 && region.start > lastEnd + 1) {
				...
				const ellipsisIndent =
					prevIndent.length < nextIndent.length ? prevIndent : nextIndent;
