...
export function findAndReplace(
	...
	if (perfectMatch.success) {
		return performReplace(
			content,
			perfectMatch.lineStart!,
			perfectMatch.lineEnd!,
			replaceContent
		);
	}

	const whitespaceMatch = findWhitespaceMatch(
		content,
		searchContent,
		startLine
	);
	if (whitespaceMatch.success) {
		return performReplace(
			content,
			whitespaceMatch.lineStart!,
			whitespaceMatch.lineEnd!,
			replaceContent
		);
	}
	...
	if (trailingMatch.success) {
		return performReplace(
			content,
			trailingMatch.lineStart!,
			trailingMatch.lineEnd!,
			replaceContent
		);
	}

	const oneLinerMatch = findOneLinerMatch(
		content,
		searchContent,
		replaceContent,
		startLine
...
export function findWhitespaceMatch(
	...
	for (let i = startLine; i <= contentLines.length - searchLines.length; i++) {
		const matches = searchLines.every((searchLine, j) => {
			...
			return (
				contentLine.replace(/\s+/g, ' ') === searchLine.replace(/\s+/g, ' ')
			);
		});
		if (matches) {
			return {
...
export function findOneLinerMatch(
	...
	for (let i = startLine; i < contentLines.length; i++) {
		if (contentLines[i].includes(searchContent)) {
			const replacedLine = contentLines[i].replace(
				searchContent,
				replaceContent
			);
...
export function findTrailingMatch(
	content: string,
	searchContent: string,
	startLine: number = 0
): MatchResult {
...
export function findDMPMatch(
	...
	if (bestMatch.length > searchContent.length * 0.7) {
		...
		const linesBefore =
			subContent.substr(0, bestMatch.start).split('\n').length - 1;
