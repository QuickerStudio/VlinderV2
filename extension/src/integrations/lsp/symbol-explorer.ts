...
export class SymbolExplorer {
	...
	public async listTopLevelFunctionIntelliSense(): Promise<string> {
		...
		>('vscode.executeDocumentSymbolProvider', this.fileUri);
		...
		const functionSymbols = symbols.filter(
			(sym) =>
				...
				(sym.children &&
					...
					functionLikeKinds.has(sym.children[0].kind)) ||
				(sym.children &&
					sym.children.length > 0 &&
		...
		const xmlFunctions = functionInfos
			.map((func) => {
				...
				const safeHover = func.hoverPreview
					.replace(/&/g, '&amp;')
					.replace(/</g, '&lt;')
					.replace(/>/g, '&gt;');
		...
		if (firstHover.contents && firstHover.contents.length > 0) {
			...
			if (text) {
				...
				return firstLine.length > 0
					? firstLine
					: 'No IntelliSense info available';
			}
		}
