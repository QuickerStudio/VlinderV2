...
export class BannerDropEditProvider implements vscode.DocumentDropEditProvider {
        ...
        try {
            ...
            await vscode.window.withProgress({
                ...
                if (!hasFormatter) {
                    ...
                    const installed = await this.promptAndInstallFormatter(document.languageId)
                    if (!installed) {
                        progress.report({ increment: 40, message: "Installation cancelled" })
                        throw new Error(`No formatter found for ${languageId} files, and user cancelled installation`)
                    }
                    progress.report({ increment: 40, message: "Formatter installation completed" })
                }
    ...
    private getFileExtension(fileName: string): string {
        if (fileName === 'Dockerfile' || fileName.toLowerCase().includes('dockerfile')) {
            return 'Dockerfile'
        }
        ...
        if (choice === 'Install Formatter') {
            try {
                ...
                return await vscode.window.withProgress({
                    ...
                    progress.report({ increment: 30, message: "Preparing installation" })

                    // Execute installation command
                    await vscode.commands.executeCommand('workbench.extensions.installExtension', formatterInfo.extensionId)

                    progress.report({ increment: 70, message: "Installation completed" })
                ...
                const errorMessage = error instanceof Error ? error.message : String(error)
                vscode.window.showErrorMessage(`Failed to install ${formatterInfo.name}: ${errorMessage}`)
                return false
            }
        }
