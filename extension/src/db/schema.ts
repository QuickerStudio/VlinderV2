...
export const tasks = sqliteTable('tasks', {
	...
	isRepoInitialized: integer('is_repo_initialized', { mode: 'boolean' })
		.notNull()
		.default(false),
...
export const taskAgents = sqliteTable(
	'task_agents',
	{
		...
		isMainThread: integer('is_main_thread', { mode: 'boolean' })
			.notNull()
			.default(true),
...
export const messages = sqliteTable(
	'messages',
	{
		...
		isFetching: integer('is_fetching', { mode: 'boolean' })
			.notNull()
			.default(false),
		...
		isSubMessage: integer('is_sub_message', { mode: 'boolean' })
			.notNull()
			.default(false),
...
export const apiHistory = sqliteTable(
	'api_history',
	{
		...
		updatedAt: integer('updated_at', { mode: 'timestamp_ms' })
			.notNull()
			.$defaultFn(() => new Date()),
		content: text('data', { mode: 'json' })
			.$type<ApiHistoryItem['content']>()
			.notNull(),
	...
	(t) => [
		index('api_history_task_id_idx').on(t.taskId),
		index('api_history_agent_name_idx').on(t.agentName),
	]
);
