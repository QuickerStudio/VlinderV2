...
const rejectMsg = (msg: string) =>
	`The Tool got rejected and returned the following message: ${msg}`;
const errorMsg = (msg: string) =>
	`The Tool encountered an error and returned the following message: ${msg}`;
const feedbackMsg = (msg: string) =>
	`The Tool returned the following feedback: ${msg}`;
...
export const toolResponseToAIState = (
...
export function getBase64ImageType(
	base64String: string
): ImageBlockParam['source']['media_type'] | null {
...
export function parseToolResponse(xmlString: string): ToolResponse {
	try {
		...
		const getTagContent = (xml: string, tag: string): string => {
			...
			if (endIndex === -1) {
				throw new Error(
					`Malformed XML: Unable to find matching end tag for ${tag}`
				);
			}
		...
		const hasImages = toolResponseContent.includes(
