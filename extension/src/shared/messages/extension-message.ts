...
type AssetUriMessage = {
	type: 'assetUri';
	assetPath: string;
	uri: string;
};

type ContextCompressionCompleteMessage = {
...
type ContextCompressionErrorMessage = {
	type: 'contextCompressionError';
	error: string;
};

type SoundUrisMessage = {
	type: 'soundUris';
	uris: {
		pop: string;
		ding: string;
...
export type ExtensionMessage =
	...
	| AssetUriMessage
	| ContextCompressionCompleteMessage
	| ContextCompressionErrorMessage
	| SoundUrisMessage
	| WorkspaceInfoMessage
	| OpenFilesInfoMessage;
...
export interface BaseExtensionState {
	...
	gitCommitterType?: 'anthropic' | 'user';
...
export const isV1ClaudeMessage = (
	message: ClaudeMessage
): message is V1ClaudeMessage => {
