...
const taskRouter = router({
	openFile: procedure
		...
		.resolve(async (ctx, input) => {
			...
			const isAvailable = await vscode.workspace.fs
				.stat(vscode.Uri.file(joinedPath))
				.then(
					() => true,
					() => false
				);
		...
		.resolve(async (ctx, input) => {
				...
				async (progress) => {
					try {
						...
						const taskDirPath = path.join(
							ctx.provider.context.globalStorageUri.fsPath,
							'tasks'
						);
						...
						const allTasks = allFolders.filter(
							(folder) => folder !== '.DS_Store'
						);
						...
						for (const task of tasksToRestore) {
							...
							try {
								...
								console.log(
									`Error reading claude_messages.json for task ${task}: ${e}`
								);
							}
							processedTasks++;
						}
						...
						progress.report({
							message: 'Done, ${taskHistoryItem.length} tasks restored',
							increment: 100,
						});
			...
			if (res.tasksToRestore.length === 0) {
				...
				await vscode.window.showInformationMessage(
					`Restored ${res.tasksToRestore.length} tasks`
				);
			}
			return res;
		}),
			...
			try {
				...
				return new Promise((resolve, reject) => {
					...
					output.on('error', (err) => {
						...
						vscode.window.showErrorMessage(
							`Failed to create output file: ${err.message}`
						);
					...
					output.on('close', () => {
						...
						vscode.window.showInformationMessage(
							`Task ${input.taskId} exported to ${savePath}`
						);
