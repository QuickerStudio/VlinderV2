...
export async function getCurrentApiSettings(): Promise<ApiConstructorOptions | null> {
	...
	const matchingProviderSettings = savedProviderSettings.find(
		(p: ProviderSettings) => p.providerId === apiConfig.providerId
	);
...
const providerRouter = router({
	getGlobalState: procedure
		.input(z.object({ key: z.string() }))
		.resolve(async (ctx, input) => {
			return GlobalStateManager.getInstance().getGlobalState(
				input.key as keyof GlobalState
			);
		}),

	updateGlobalState: procedure
	...
	currentObserverModel: procedure
		.input(z.object({}))
		.resolve(async (ctx, input) => {
			const observerModel =
				GlobalStateManager.getInstance().getGlobalState('observerSettings');
			if (!observerModel) {
	...
	selectObserverModel: procedure
		...
		.resolve(async (ctx, input) => {
			await GlobalStateManager.getInstance().updatePartialGlobalState(
				'observerSettings',
				{
	...
	selectModel: procedure
		...
		.resolve(async (ctx, input) => {
			...
			if (!modelExists) {
				throw new Error(
					`Invalid model for provider ${input.providerId}: ${input.modelId}`
				);
			}
	...
	currentScholarModel: procedure
		.input(z.object({}))
		.resolve(async (ctx, input) => {
			const scholarModel =
				GlobalStateManager.getInstance().getGlobalState('scholarSettings');
			if (!scholarModel) {
