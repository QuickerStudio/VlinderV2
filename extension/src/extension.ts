...
export function activate(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			`${extensionName}.plusButtonTapped`,
			async () => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			`${extensionName}.popoutButtonTapped`,
			openExtensionInNewTab
		)
	);
	context.subscriptions.push(
		vscode.commands.registerCommand(
			`${extensionName}.openInNewTab`,
			openExtensionInNewTab
		)
	);
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			`${extensionName}.settingsButtonTapped`,
			() => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			`${extensionName}.historyButtonTapped`,
			() => {
				sidebarProvider?.getWebviewManager()?.postMessageToWebview({
					type: 'action',
					action: 'historyButtonTapped',
				});
			}
		)
	);
	...
	*/
	...
	const modifiedContentProvider = new (class
		implements vscode.TextDocumentContentProvider
	{
	...
	const inlineDiffContentProvider = new (class
		implements vscode.TextDocumentContentProvider
	{
		provideTextDocumentContent(uri: vscode.Uri): string {
			return Buffer.from(uri.query, 'base64').toString('utf-8');
	...
	const inlineModifiedContentProvider = new (class
		implements vscode.TextDocumentContentProvider
	{
		private content = new Map<string, string>();
	...
	(vscode.workspace.registerTextDocumentContentProvider(
		INLINE_DIFF_VIEW_URI_SCHEME,
		inlineDiffContentProvider
	),
		vscode.workspace.registerTextDocumentContentProvider(
			INLINE_MODIFIED_URI_SCHEME,
			inlineModifiedContentProvider
		));
