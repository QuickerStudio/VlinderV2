...
import { ScholarHook } from './hooks/scholar-hook';
import { hasPendingTimerNotifications, formatTimerNotifications } from './tools/runners/timer.tool';
import { GlobalStateManager } from '../../providers/state/global-state-manager';
import dedent from 'dedent';
import { nanoid } from 'nanoid';

// Simple Timer Notification Hook that uses timer.tool.ts functions
class TimerNotificationHook extends BaseHook {
	constructor(options: HookOptions, MainAgent: MainAgent) {
		super(options, MainAgent);
	}

	protected shouldExecute(): boolean {
		return hasPendingTimerNotifications();
	}

	protected async executeHook(): Promise<string | null> {
		return formatTimerNotifications();
	}
}
...
export class MainAgent {
	...
	constructor(
		...
		const triggerEvery =
			GlobalStateManager.getInstance().getGlobalState(
				'observerSettings'
			)?.observeEveryXRequests;
		const scholarTriggerEvery =
			GlobalStateManager.getInstance().getGlobalState(
				'scholarSettings'
			)?.scholarEveryXRequests;
		...
		if (!options.noTask && scholarTriggerEvery) {
			// Add scholar hook for automatic knowledge extraction
			// Scholar Hook triggers on task completion, not on triggerEvery
			this.hookManager.registerHook(ScholarHook, {
				hookName: 'scholar',
				triggerEvery: 1, // Always check, but shouldExecute() controls actual execution
				autoTrigger: true,
			} as any);
		}

		// Always register TimerNotificationHook - it only triggers when there are pending notifications
		if (!options.noTask) {
			this.hookManager.registerHook(TimerNotificationHook, {
				hookName: 'timer',
				triggerEvery: 1, // Always check, but shouldExecute() controls actual execution
				autoTrigger: true, // Hook controls its own triggering via shouldExecute()
			});
		}
	...
	async resumeTaskFromHistory() {
		...
		const modifiedClaudeMessages =
			await this.stateManager.claudeMessagesManager.getSavedClaudeMessages();
		...
		const isCompleted =
			...
			(lastClaudeMessage &&
				...
				(JSON.parse(lastClaudeMessage.text ?? '{}') as ChatTool).tool ===
					'attempt_completion');
		...
		const modifiedClaudeMessagesAfterResume =
			await this.stateManager.claudeMessagesManager.getSavedClaudeMessages();
		const lastAskIndex = modifiedClaudeMessagesAfterResume.findIndex(
			(m) => m.ts === ts
		);
		if (lastAskIndex !== -1) {
		...
		await this.stateManager.claudeMessagesManager.overwriteClaudeMessages(
			modifiedClaudeMessagesAfterResume
		);
		await this.providerRef
			.deref()
			?.getWebviewManager()
		...
		const modifiedApiConversationHistory =
			existingApiConversationHistory.filter((m) => {
				let shouldKeep = true;
		...
		await this.stateManager.apiHistoryManager.overwriteApiConversationHistory(
			modifiedApiConversationHistory
		); // remove the corrupted messages

		const newUserContentText = newUserContent.find(
			(block) => block.type === 'text'
		)?.text;
		// if it's been more than 15 minutes since the last message, we should reload the files
		const shouldReloadFiles =
			lastClaudeMessage && Date.now() - lastClaudeMessage.ts > 15 * 60 * 1000;
		const agoText = (() => {
		...
		const pastRequestsCount = modifiedApiConversationHistory.filter(
			(m) => m.role === 'assistant'
		).length;
		amplitudeTracker.taskResume(
			this.stateManager.state.taskId,
			pastRequestsCount
		);
	...
	observerHookEvery(value?: number) {
		...
		if (this.hookManager.hasHook('observer')) {
			this.hookManager
				.getHook('observer')
				?.updateOptions({ triggerEvery: value });
		} else {
			this.hookManager.registerHook(ObserverHook, {
				hookName: 'observer',
				triggerEvery: value,
	...
	scholarHookEvery(value?: number) {
		// if not value is provided, we disable the scholar hook
		if (!value) {
			this.hookManager.removeHook('scholar');
			return;
		}
		// check if the scholar hook is already registered
		if (this.hookManager.hasHook('scholar')) {
			// Scholar Hook doesn't use triggerEvery for frequency, but we keep it for enable/disable
			this.hookManager.getHook('scholar')?.updateOptions({ triggerEvery: 1 });
		} else {
			this.hookManager.registerHook(ScholarHook, {
				hookName: 'scholar',
				triggerEvery: 1, // Always check, but shouldExecute() controls actual execution
	...
	/**
	 * Request Scholar Agent services (like memory systems in popular coding assistants)
	 */
	async requestScholarService(
		type: 'extract' | 'search' | 'summarize',
		context?: string
	): Promise<string | null> {
		const scholarHook = this.hookManager.getHook('scholar') as ScholarHook;
		if (!scholarHook) {
			console.warn('[MainAgent] - Scholar Hook not available');
			return null;
		}
		return await scholarHook.requestScholarService(type, context);
	}
	...
	async markAsUncompleted() {
		await this.providerRef
			.deref()
			?.getTaskManager()
			.markTaskAsUncompleted(this.stateManager.taskId);
		await this.providerRef
			.deref()
			?.getWebviewManager()
			.postBaseStateToWebview();
	...
	async getEnvironmentDetails(includeFileDetails: boolean = true) {
		let details = '';
		const lastTwoMsgs =
			this.stateManager.state.apiConversationHistory.slice(-2);
		...
		const isLastMsgMutable = lastTwoMsgs.some((msg) => {
			if (Array.isArray(msg.content)) {
				return msg.content.some((block) => {
					if (block.type === 'text') {
						const isInclude = awaitRequierdTools.some((i) =>
							block.text.includes(`<${i}>`)
						);
						return isInclude;
					}
					return false;
			...
			if (typeof msg.content === 'string') {
				return awaitRequierdTools
					.map((i) => `<${i}>`)
					.some((i) => `${msg.content}`.includes(i));
			}
			return false;
		});

		const diagnosticsHandler = DiagnosticsHandler.getInstance();
		const files = this.stateManager.historyErrors
			? Object.keys(this.stateManager.historyErrors)
			: [];
		let isShowingDiagnosticsTs: number | undefined;
		if (isLastMsgMutable && files.length > 0) {
			if (this.taskExecutor.currentStreamTs) {
				await this.taskExecutor.updateSayPartial(
					this.taskExecutor.currentStreamTs,
					{
						diagnostics: {
							results: [],
							state: 'pending',
		...
		const newErrors = diagnostics.filter(
			(diag) =>
				diag.errorString !== null &&
				diag.errorString !== undefined &&
				diag.errorString !== ''
		);
		const taskErrorsRecord = newErrors.reduce(
			(acc, curr) => {
				acc[curr.key] = {
		...
		if (newErrors.length === 0) {
			...
			console.log(
				`[ENVIRONMENT DETAILS] Errors found:`,
				newErrors.map((diag) => diag.errorString).join('\n')
			);
		}
		...
		if (newErrors.length === 0) {
			...
			console.log(
				'[ENVIRONMENT DETAILS] New errors found',
				newErrors.map((diag) => diag.errorString).join('\n')
			);
