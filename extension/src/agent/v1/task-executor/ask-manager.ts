...
import {
	ChatTool,
	mustRequestApprovalTools,
	mustRequestApprovalTypes,
	readOnlyTools,
} from '../../../shared/new-tools';
...
export class AskManager {
	...
	public async abortPendingAsks(): Promise<void> {
		...
		if (this.currentAsk) {
			const askData = this.stateManager.claudeMessagesManager.getMessageById(
				this.currentAskId!
			);
			if (askData) {
				try {
					...
					if (typeof tool === 'object') {
						...
						skipUpdate = true;
						await this.stateManager.claudeMessagesManager.deleteClaudeMessage(
							this.currentAskId!,
							true
						);
					}
	...
	public handleResponse(
		...
		if (this.currentAsk) {
			console.log(
				`No exact match found for response ${id}, using current ask ${this.currentAskId}`
			);
			this.resolveCurrentAsk(response, text, images);
			return;
		}
	...
	private async handleExistingAskUpdate(
		askTs: number,
		type: ClaudeAsk,
		tool?: ChatTool
	): Promise<AskResponse> {
	...
	private async updateState(
		...
		try {
			if (this.stateManager.claudeMessagesManager.getMessageById(id)) {
				...
				await this.stateManager.claudeMessagesManager.addToClaudeMessages(
					message,
					true
				);
			}
	...
	private shouldAutoApprove(type: ClaudeAsk, tool?: string): boolean {
		if (
			this.stateManager.alwaysAllowReadOnly &&
			tool &&
			readOnlyTools.includes(tool as any)
		) {
			return true;
		}
