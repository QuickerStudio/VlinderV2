...
export abstract class TaskExecutorUtils {
	...
	public async handleAskResponse(
		response: ClaudeAskResponse,
		text?: string,
		images?: string[]
	): Promise<void> {
	...
	public async updateSayPartial(
		sayTs: number,
		data: DeepPartial<ClaudeMessage>
	): Promise<void> {
		const message =
			this.stateManager.claudeMessagesManager.getMessageById(sayTs);
		if (!message) {
			return;
		}
