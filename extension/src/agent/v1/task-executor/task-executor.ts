...
// Define SSE response type locally
...
export class TaskExecutor extends TaskExecutorUtils {
	...
	private async flushTextBuffer(currentReplyId?: number | null) {
		...
		if (
			lastAskTs &&
			lastAskTs > currentReplyId &&
			contentToFlush.trim().length > 0
		) {
			// Create new message to maintain order
			this._currentReplyId = await this.say(
				'text',
				contentToFlush,
				undefined,
				Date.now(),
			...
			void this.stateManager.providerRef
				.deref()
				?.getWebviewManager()
				?.postBaseStateToWebview();
	...
	public async newMessage(message: UserContent) {
		...
		const images = message
			...
			.map((item) => item.source.data);

		await this.say(
			'user_feedback',
			message[0].type === 'text' ? message[0].text : 'New message',
			images
		);
	...
	public async resumeTask(userContent: UserContent): Promise<void> {
		...
		if (this.state === TaskState.WAITING_FOR_USER) {
			...
			this.logError(
				new Error(
					'Cannot resume task: not in WAITING_FOR_USER state'
				) as TaskError
			);
		}
	}
	...
	private normalizeUserContent(content: UserContent): UserContent {
		if (
			content.length === 0 ||
			(content[0]?.type === 'text' && !content[0].text?.trim())
		) {
			return [
				{
					type: 'text',
	...
	private async cancelCurrentRequest(): Promise<void> {
		...
		if (
			lastApiRequest &&
			isV1ClaudeMessage(lastApiRequest) &&
			!lastApiRequest.isDone
		) {
			const msg =
				await this.stateManager.claudeMessagesManager.updateClaudeMessage(
					lastApiRequest.ts,
					{
						...lastApiRequest,
						isDone: true,
		...
		this.ask('resume_task', {
			...
			if (res.response === 'yesButtonTapped') {
				...
				(res.response === 'noButtonTapped' && res.text) ||
				res.images
			) {
		...
		try {
			...
			const systemPrompt = this.stateManager.subAgentManager.state?.systemPrompt
				? {
						...
						automaticReminders:
							this.stateManager.subAgentManager.state?.automaticReminders,
					}
				: undefined;
				...
				if (error instanceof ApiError) {
					...
					if (error.errorCode === 401) {
						await this.handleApiError(
							new TaskError({ type: 'UNAUTHORIZED', message: error.message })
						);
						return;
					}
					// Payment error handling removed - unlimited access
					await this.handleApiError(
						new TaskError({ type: 'API_ERROR', message: error.message })
					);
					return;
				}
				if (`${error}`.includes('garbage collected')) {
					console.log(
						'[TaskExecutor] Provider was garbage collected, ignoring error'
					);
					return;
				}
				await this.handleApiError(
	...
	private async processApiResponse(
		stream: AsyncGenerator<SSEResponse, any, unknown>,
		startedReqId: number
	): Promise<void> {
		if (
			this.state !== TaskState.PROCESSING_RESPONSE ||
					...
					if (chunk.code === 1) {
						...
						const msg =
							await this.stateManager.claudeMessagesManager.updateClaudeMessage(
								startedReqId,
								{
									...this.stateManager.claudeMessagesManager.getMessageById(
										startedReqId
									)!,
									apiMetrics: {
				...
				onChunk: async (chunk) => {
					...
					if (chunk.code === 2) {
						if (!firstChunkTs) {
							...
							const currentReplyId = await this.say(
								'text',
								'',
								undefined,
								firstChunkTs,
						...
						if (
							Array.isArray(apiHistoryItem.content) &&
							isTextBlock(apiHistoryItem.content[0])
						) {
						...
						if (!this.streamPaused) {
							...
							const nonXMLText =
								await this.toolExecutor.processToolUse(accumulatedText);
							...
							if (isChunkInsideTool && !this.toolExecutor.hasActiveTools()) {
								...
								const afterClosingTag = accumulatedText
									.split('</')[1]
									?.split('>')[1];
								if (afterClosingTag) {
	...
	private async finishProcessingResponse(
		assistantResponses: ApiHistoryItem
	): Promise<void> {
		...
		if (
			...
			(isTextBlock(assistantResponses.content[0]) &&
				!assistantResponses.content[0].text.trim())
		) {
		...
		// Special handling: if the last timer was explicitly stopped, do not restart it.
		// Instead, instruct the agent to decide between asking a follow-up question or completing the task.
		try {
			const lastTimerMsg = this.stateManager.state.claudeMessages
				.slice()
				.reverse()
				.find((msg) => {
					if (!isV1ClaudeMessage(msg) || msg.ask !== 'tool') {return false;}
					try {
						const parsed = JSON.parse(msg.text || '{}') as ChatTool;
						return parsed.tool === 'timer' && parsed.approvalState === 'approved';
					} catch {
						return false;
					}
				});

			if (lastTimerMsg && isV1ClaudeMessage(lastTimerMsg)) {
				let parsed: ChatTool | null = null;
				try {
					parsed = JSON.parse(lastTimerMsg.text || '{}') as ChatTool;
				} catch {}
				if (parsed && parsed.tool === 'timer' && (parsed as any).timerStatus === 'stopped') {
					// Check if we've already injected the stopped timer message
					const lastUserMessage = this.stateManager.state.apiConversationHistory
						.slice()
						.reverse()
						.find((msg) => msg.role === 'user');

					const stoppedTimerMessageText = 'The timer was stopped by the user. Do not restart or resume this timer.';
					let alreadyInjected = false;

					if (lastUserMessage?.content && Array.isArray(lastUserMessage.content)) {
						alreadyInjected = lastUserMessage.content.some(
							(block) =>
								block.type === 'text' &&
								'text' in block &&
								typeof block.text === 'string' &&
								block.text.includes(stoppedTimerMessageText)
						);
					}

					// Only inject the message once
					if (!alreadyInjected) {
						// Prepare directive for the agent to choose next action without restarting the timer
						this.state = TaskState.WAITING_FOR_API;
						this.currentUserContent = [
							{
								type: 'text',
								text:
									`The user has stopped the timer. Do not start a new timer after it has been stopped unless there are special circumstances or a specific task. You should independently analyze the current situation and make the best choice, focusing on finding practical ways to move the task forward.`,
							},
						];
						await this.makeClaudeRequest();
						return;
			...
			// Non-fatal: continue normal flow if inspection fails
		}

		if (currentToolResults.length > 0) {
			const completionAttempted = currentToolResults.find(
				(result) => result?.name === 'attempt_completion'
			);
			...
			if (
				completionAttempted &&
				completionAttempted.result.status === 'success'
			) {
				await this.stateManager.apiHistoryManager.addToApiConversationHistory({
					role: 'user',
			...
			this.currentUserContent = [
				{
					type: 'text',
					text: `You must use a tool to proceed. Either use attempt_completion if you've completed the task, or ask_followup_question if you need more information. you must adhere to the tool format <action><tool_name><parameter1_name>value1</parameter1_name><parameter2_name>value2</parameter2_name>... additional parameters as needed in the same format ...</tool_name></action>`,
				},
			];
			await this.makeClaudeRequest();
		}
	...
	private async handleApiError(
		error: TaskError | CustomProviderError
	): Promise<void> {
		this.logError(error);
		...
		if (lastMessage?.role === 'assistant' && lastMessage.ts) {
			...
			if (
				...
				isTextBlock(lastMessage.content[0])
			) {
		...
		if (
			...
			lastMessage.ts &&
			lastMessage.ts === this._currentStreamTs
		) {
			// remove the last message from the history
			await this.stateManager.apiHistoryManager.deleteApiHistoryItem(
				lastMessage.ts
			);
		}
		const modifiedClaudeMessages =
			this.stateManager.state.claudeMessages.slice();
		// clean ui messages
		cleanUIMessages(modifiedClaudeMessages, error);

		// Process state updates in parallel

		await this.stateManager.claudeMessagesManager.overwriteClaudeMessages(
		...
		if (
			...
			(error.type === 'PAYMENT_REQUIRED' || error.type === 'UNAUTHORIZED')
		) {
			this.state = TaskState.IDLE;
			this.say(
				error.type === 'PAYMENT_REQUIRED' ? 'payment_required' : 'unauthorized',
				error.message
			);
			return;
		}
	...
	private fixUserContent(content: UserContent): UserContent {
		if (content.length === 0) {
			return [
				{
					type: 'text',
					text: "The user didn't provide any content, please continue",
				},
			];
		}
	...
	private async handleWaitingForUser() {
		this.ask('resume_task', {
			...
			if (res.response === 'messageResponse') {
				...
				let imageBlocks: Anthropic.ImageBlockParam[] = formatImagesIntoBlocks(
					res.images
				);
