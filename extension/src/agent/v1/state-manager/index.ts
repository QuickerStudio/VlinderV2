...
export class StateManager {
	...
	constructor(options: MainAgentOptions, apiManager: ApiManager) {
		...
		const globalStoragePath =
			this._providerRef.deref()?.context.globalStorageUri.fsPath;
		...
		if (['full', 'diff'].includes(options.inlineEditOutputType ?? '')) {
			...
			provider
				...
				.updateGlobalState('inlineEditOutputType', 'full');
		}
	...
	public setState(newState: AgentState): void {
		...
		if (newState.apiConversationHistory) {
			this._state.apiConversationHistory.push(
				...newState.apiConversationHistory
			);
		}
	...
	public setHistoryErrorsEntry(
		key: string,
		value: NonNullable<AgentState['historyErrors']>[string]
	): void {
	...
	private async onEnterSuccesfulSubAgent(
	...
	public async getFilesInTaskDirectory(): Promise<
