...
export class ApiHistoryManager {
	...
	public async getSavedApiConversationHistory(
		fromDisk = false
	...
	public async deleteApiHistoryItem(messageId: number) {
		const index = this.state.apiConversationHistory.findIndex(
			(msg) => msg?.ts === messageId
		);
		if (index === -1) {
			console.error(
				`[ApiHistoryManager] deleteApiConversationHistory: Message with id ${messageId} not found`
			);
			return;
		}
	...
	public async updateApiHistoryItem(
		messageId: number,
		...
		if (index === -1) {
			console.error(
				`[ApiHistoryManager] updateApiConversationHistory: Message with id ${messageId} not found`
			);
			return;
		}
