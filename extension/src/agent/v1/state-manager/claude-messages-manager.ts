...
export class ClaudeMessagesManager {
	...
	public async saveClaudeMessages(
		...
		try {
			let lastMessage =
				this.state.claudeMessages[this.state.claudeMessages.length - 1];
			...
			if (
				...
				lastMessage =
					this.state.claudeMessages[this.state.claudeMessages.length - 2];
			}
			...
			if (lastMessage.ask === 'tool') {
				...
				if (
					toolData.tool === 'attempt_completion' &&
					toolData.approvalState === 'approved'
				) {
		...
		// Calculate elapsed time from completed messages
		const elapsedTime = this.state.claudeMessages.reduce((acc, msg) => {
			...
			.updateTaskHistory(
				{
					...
					elapsedTime: elapsedTime > 0 ? elapsedTime : undefined,
				},
				{
	...
	public async deleteClaudeMessage(messageId: number, withFlush = true) {
		const index = this.state.claudeMessages.findIndex(
			(msg) => msg?.ts === messageId
		);
		if (index === -1) {
			console.error(
				`[ClaudeMessagesManager] deleteClaudeMessage: Message with id ${messageId} not found`
			);
			return;
		}
		...
		if (withFlush) {
			...
			await provider
				?.getWebviewManager()
				?.postClaudeMessagesToWebview(this.state.claudeMessages);
		}
		return this.state.claudeMessages;
	}
	...
	public async addToClaudeMessages(message: ClaudeMessage, withFlush = true) {
		...
		if (withFlush) {
			await this.providerRef
				.deref()
				?.getWebviewManager()
				.postClaudeMessageToWebview(message);
		}
		return message;
	}
	...
	public async removeEverythingAfterMessage(
		messageId: number,
		withFlush = true
	) {
		const index = this.state.claudeMessages.findIndex(
	...
	// Remove the target message and everything after it (including the target message itself)
	public async removeEverythingFromMessage(
		messageId: number,
		withFlush = true
	) {
		const index = this.state.claudeMessages.findIndex(
		...
		if (index === -1) {
			console.error(
				`[ClaudeMessagesManager] removeEverythingFromMessage: Message with id ${messageId} not found`
			);
			return;
		}
		// In-place modification using .length - truncate from the target message
		this.state.claudeMessages.length = index;
		await this.saveClaudeMessages();
		if (withFlush) {
			const provider = this.safeProviderRef();
			await provider
	...
	public async updateClaudeMessage(
		messageId: number,
		...
		if (index === -1) {
			console.error(
				`[ClaudeMessagesManager] updateClaudeMessage: Message with id ${messageId} not found`
			);
			return;
		}
		...
		if (withFlush) {
			await this.safePostMessage(message).catch((err) =>
	...
	public async appendToClaudeMessage(
		messageId: number,
		text: string,
		withFlush = true
	) {
		const lastMessage = this.state.claudeMessages.find(
			(msg) => msg?.ts === messageId
		);
	...
	public async addToClaudeAfterMessage(
		messageId: number,
		message: ClaudeMessage,
		withFlush = true
	) {
		...
		if (index === -1) {
			console.error(
				`[ClaudeMessagesManager] addToClaudeAfterMessage: Message with id ${messageId} not found`
			);
			return;
		}
		if (isV1ClaudeMessage(message)) {
