...
export const renamePrompt: ToolPromptSchema = {
	name: "rename",
	description: "Rename files and directories in place without changing their location. Automatically detects whether the target is a file or directory.",
	parameters: {
		path: {
			type: "string",
			description: "The current path of the file or directory to rename (relative to the current working directory).",
			required: true,
		},
		new_name: {
			type: "string",
			description: "The new name for the file or directory (just the name, not the full path).",
			required: true,
		},
		type: {
			type: "string",
		...
		overwrite: {
			type: "boolean",
			description: "Whether to overwrite if a file or directory with the new name already exists in the same location. Default is false.",
			required: false,
		},
	},
	capabilities: [
