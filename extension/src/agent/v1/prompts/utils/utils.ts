...
export type ValidTemplateString<T extends string> =
	ExtractPlaceholders<T> extends never ? never : T;
...
export function promptTemplate(
	...
	const builder = Object.fromEntries(
		templatePlaceHolder.map((p) => [p, `{{${p}}}`])
	) as Record<TemplatePlaceholder, string>;
...
export async function buildPromptFromTemplate(
	...
	const vision =
		(await ApiManager.getInstance().getCurrentModelInfo())?.supportsImages ??
		false;
