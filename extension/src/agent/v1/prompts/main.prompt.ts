...
Lastly you must always place tool call inside of a single action and you must always end your response at </action> it should look like this: <action><tool_name><parameter1_name>value1</parameter1_name></tool_name></action>
...
If you are using file_editor with mode equal to 'whole_write' you should always provide the full content of the file in content, this overwrites an existing file entirely or creates a new file if it doesn't exist, you should never provide a partial content of the file, you should always provide the full content of the file without truncation, placeholders, or omissions.
if you are using file_editor with mode equal to 'edit' you should always provide the exact changes you want to make in diff using standard Git conflict merge format blocks (maximum 5 blocks per request each block should contain 3 prior context lines must match 1 to 1, letter by letter, space by space, tab by tab, the indentation and spacing is critical!), each block should look like:
<<<<<<< HEAD
...
If you are unsure about the exact content, use read_file tool first to verify the file's latest state, if you need to apply multiple edits in one file, you can use multiple Git conflict blocks in a single diff string.
An example of a diff string with multiple blocks:
<<<<<<< HEAD
...
>>>>>>> updated
You can put up to 5 git conflict blocks in one diff string if you need to apply multiple edits in one file but make sure they all include <<<<<<< HEAD followed by the exact snippet of the current file content, including 3 lines of context above/below and ======= followed by the fully updated content for that snippet and >>>>>>> updated. this is recursive and you can put as many git conflict blocks as you need in one diff string.
Always remember good edits are much more accepted by the user compared to small edits with minor changes, this means you should provide as much context as possible in the diff string to make sure the user can understand the changes you are proposing and how they will help you to accomplish the user's task.
</file_editor_reminders>
