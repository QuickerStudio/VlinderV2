...
export class GitHandler {
	...
	private async getCommitterInfo(): Promise<{ name: string; email: string }> {
		const committerType =
			GlobalStateManager.getInstance().getGlobalState('gitCommitterType') ??
			'Vlinder';
		...
		try {
			const name =
				(await this.getLocalConfigValue('user.name')) ||
				(await this.getGlobalConfigValue('user.name'));
			const email =
				(await this.getLocalConfigValue('user.email')) ||
				(await this.getGlobalConfigValue('user.email'));
	...
	private checkEnabled(): boolean {
		const shouldCommit =
			GlobalStateManager.getInstance().getGlobalState('gitHandlerEnabled');
		if (!shouldCommit) {
	...
	private async commitWithMessage(
		...
		try {
			...
			const { stdout } = await execa(
				'git',
				['commit', '--author', `${name} <${email}>`, '-m', message],
				{
					cwd: this.repoPath,
				}
			);
			...
			if (error instanceof ExecaError) {
				console.error(
					`Error committing changes: ${error.stderr || error.message}`
				);
				throw new Error(
					`Error committing changes: ${error.stderr || error.message}`
				);
	...
	async createBranchAtCommit(
		...
		try {
			await execa('git', ['branch', branchName, commitHash], {
				cwd: this.repoPath,
			});
			return true;
		} catch (error) {
	...
	private async getLocalConfigValue(key: string): Promise<string | null> {
		try {
			const { stdout } = await execa('git', ['config', '--local', key], {
				cwd: this.repoPath,
			});
			return stdout.trim();
		} catch (error) {
	...
	async revertToCommit(
		...
		if (!this.repoPath || !this.checkEnabled()) {
			return {
				success: false,
				error: 'Git handler is disabled or no repository path',
			};
		}
