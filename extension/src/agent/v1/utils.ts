...
import { normalizePathEncodingExport } from '../../utils/path-helpers';
...
export function getReadablePath(
	relPath: string,
	customCwd: string = cwd
): string {
	// Normalize encoding for both paths to handle non-ASCII characters properly
	relPath = normalizePathEncodingExport(relPath);
	customCwd = normalizePathEncodingExport(customCwd);
...
export function formatFilesList(
	...
	// Normalize encoding for the absolute path to handle non-ASCII characters properly
	absolutePath = normalizePathEncodingExport(absolutePath);

	const sorted = files
		.map((file) => {
			// Normalize encoding for each file path and convert absolute path to relative path
			file = normalizePathEncodingExport(file);
 ...
 */
export function formatImagesIntoBlocks(
	images?: string[]
 ...
 * @returns <t
 */
export function formatToolResponseText(
	toolName: string,
...
export const isImageBlock = (
	block: any
): block is Anthropic.ImageBlockParam => {
	if (typeof block === 'object') {
	...
	messages.forEach((m, index) => {
		if (isV1ClaudeMessage(m)) {
			...
			if (m.say === 'api_req_started' && m.isFetching) {
				...
				m.errorText =
					error?.message ??
					'Task was interrupted before this API request could be completed.';
			...
			if (m.ask === 'tool' && m.type === 'ask') {
				try {
					...
					if (
						...
						parsedTool.tool === 'attempt_completion'
					) {
