...
export const SpawnAgentOptions = ['coder', 'planner', 'sub_task', 'tester'] as const;
export type SpawnAgentOptions = (typeof SpawnAgentOptions)[number];
const schema = z.object({
	agentName: z
		.enum(SpawnAgentOptions)
		.describe('Name of the sub-agent for identification'),
	instructions: z.string().describe('Instructions for the sub-agent'),
	files: z
		.string()
		.optional()
		.describe('Files to be processed by the sub-agent'),
});
