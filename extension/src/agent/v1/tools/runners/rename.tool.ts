...
export class RenameTool extends BaseAgentTool<RenameToolParams> {
	async execute() {
		...
		const { path: targetPath, new_name, type = "auto", overwrite = false } = input

		if (!targetPath?.trim()) {
			await say("error", "Vlinder tried to use rename without value for required parameter 'path'. Retrying...")
			const errorMsg = `
			<rename_response>
				<status>
					<result>error</result>
		...
		if (!new_name?.trim()) {
			await say("error", "Vlinder tried to use rename without value for required parameter 'new_name'. Retrying...")
			const errorMsg = `
			<rename_response>
				<status>
					<result>error</result>
		...
		try {
			...
			try {
				...
				await say("error", `Target path does not exist: ${getReadablePath(targetPath, this.cwd)}`)
				const errorMsg = `
				<rename_response>
					<status>
						<result>error</result>
			...
			if (!isDirectory && !isFile) {
				await say("error", `Target path is neither a file nor a directory: ${getReadablePath(targetPath, this.cwd)}`)
				const errorMsg = `
				<rename_response>
					<status>
						<result>error</result>
			...
			if (type !== "auto" && type !== actualType) {
				await say("error", `Type mismatch: specified '${type}' but target is a '${actualType}'`)
				const errorMsg = `
				<rename_response>
					<status>
						<result>error</result>
			...
			if (destinationExists && !overwrite) {
				await say("error", `A ${actualType} with the name '${new_name}' already exists and overwrite is not enabled`)
				const errorMsg = `
				<rename_response>
					<status>
						<result>error</result>
