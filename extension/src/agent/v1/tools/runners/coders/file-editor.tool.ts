...
export class FileEditorTool extends BaseAgentTool<FileEditorToolParams> {
	...
	constructor(
		params: FullToolParams<FileEditorToolParams>,
...
`;
	...
	public async handlePartialUpdateDiff(
		relPath: string,
		diff: string
	): Promise<void> {
	...
	private async _handlePartialUpdateDiff(
		relPath: string,
		diff: string
	): Promise<void> {
		...
		try {
			newBlocks = this.diffBlockManager.parseAndMergeDiff(
				diff,
		...
		if (lastBlock.isFinalized) {
			...
			this.logger(
				`Block is finalized, applying final content for block ID=${lastBlock.id}`,
				'debug'
			);
			await this.inlineEditor.applyFinalContent(
				lastBlock.id,
	...
	public async handlePartialUpdate(
		relPath: string,
		acculmatedContent: string
	): Promise<void> {
		...
		if (this.isProcessingFinalContent) {
			this.logger(
				'Skipping partial update because the tool is processing the final content.',
				'warn'
			);
			return;
		}
	...
	private async finalizeInlineEdit(
		path: string,
		content: string,
		allowFixed = true
		...
		this.logger(
			`Failed count: ${failedCount}, isAllFailed: ${isAllFailed}`,
			'debug'
		);
		if (isAnyFailed) {
...
`
		...
		try {
			commitResult = await this.MainAgent.gitHandler.commitOnFileWrite(
				path,
				this.params.input.commit_message
			);
		...
		const currentOutputMode =
			this.MainAgent.getStateManager().inlineEditOutputType;
...
${finalContent}
	...
	private async finalizeFileEdit(
		relPath: string,
		content: string
	): Promise<ToolResponseV2> {
		// determine mode based on whether content is entire or partial
		// If content is provided from input, it likely means whole content replacement. If diff, it's edit.
...
${commitXmlInfo}
		...
		if (
			...
			this.logger(
				`Truncated content detected in ${relPath} at ${this.ts}`,
				'warn'
			);
...
${commitXmlInfo}`;
		...
		if (userEdits) {
			await this.params.say(
				'user_feedback_diff',
				JSON.stringify({
					tool: this.fileState?.isExistingFile
						? 'editedExistingFile'
						: 'newFileCreated',
	...
	private async processFileWrite() {
		try {
			...
			if (diff) {
				...
				throw new Error("Missing required parameter 'content' or 'diff'");
			}
		} catch (error) {
			...
			this.params.updateAsk(
				'tool',
				{
					tool: {
						...
						mode,
						content: this.params.input.content ?? undefined,
						diff: this.params.input.diff ?? undefined,
						approvalState: 'error',
						ts: this.ts,
	...
	private async showChangesInDiffView(
		relPath: string,
		content: string
	): Promise<void> {
	 ...
	 */
	private async saveNewFileVersion(
		relPath: string,
		content: string
	): Promise<FileVersion> {
		const versions =
			await this.MainAgent.getStateManager().getFileVersions(relPath);
	...
	private async handleRollback(relPath: string): Promise<ToolResponseV2> {
		...
		if (response !== 'yesButtonTapped') {
			...
			return this.toolResponse(
				'rejected',
				'Rollback operation cancelled by user.'
			);
		}
