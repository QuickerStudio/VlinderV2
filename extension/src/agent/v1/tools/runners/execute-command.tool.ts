	...
	private async executeShellTerminal(command: string): Promise<ToolResponseV2> {
		...
		if (response !== 'yesButtonTapped') {
			...
			if (response === 'messageResponse') {
				...
				return this.toolResponse(
					'feedback',
					this.formatToolDeniedFeedback(text),
					images
				);
			}
		...
		try {
					...
					await this.params.updateAsk(
						'tool',
						{
							tool: {
								...
								ts: this.ts,
								error:
									'Shell integration is not available, cannot read output.',
								earlyExit: undefined,
								isSubMsg: this.params.isSubMsg,
							},
						},
			...
			const timeout =
				GlobalStateManager.getInstance().getGlobalState('commandTimeout');
			...
			if (
				(userFeedback?.text && userFeedback.text.length) ||
				userFeedback?.images?.length
			) {
				await this.params.updateAsk(
					'tool',
					{
			...
			return this.toolResponse(
				'error',
				this.formatToolError(`Error executing command:\n${errorMessage}`)
			);
		}
	}
}
