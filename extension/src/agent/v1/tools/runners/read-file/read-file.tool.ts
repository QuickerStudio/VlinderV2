...
export class ReadFileTool extends BaseAgentTool<ReadFileToolParams> {
	async execute() {
		...
		try {
			...
			)}</message><path>${relPath}</path><help><example_usage><action>${
				readFilePrompt.examples[0].output
			}</action></example_usage><note>Please provide a valid file path. File reading operations require a valid path parameter.</note></help></error_details></file_read_response>`;
