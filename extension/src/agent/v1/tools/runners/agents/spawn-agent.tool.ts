...
import { TESTER_SYSTEM_PROMPT } from '../../../prompts/agents/tester.prompt';
	...
	async execute() {
		...
		if (response !== 'yesButtonTapped') {
			...
			return this.toolResponse(
				'error',
				'Sub-agent operation cancelled by user.'
			);
		}
		await this.params.updateAsk(
			'tool',
		...
		switch (agentName) {
			case 'planner':
				systemPrompt = PLANNER_SYSTEM_PROMPT(
					this.MainAgent.getApiManager().getModelInfo()?.supportsImages
				);
				break;
			case 'sub_task':
		...
		await this.MainAgent.getStateManager().subAgentManager.spawnSubAgent(
			this.ts,
			{
				name: agentName,
				state: 'RUNNING',
