...
import {
	TerminalManager,
	TerminalRegistry,
} from '../../../../integrations/terminal/terminal-manager';
...
export class DevServerTool extends BaseAgentTool<ServerRunnerToolParams> {
	...
	async execute() {
		...
		if (!commandType || !serverName) {
			await say(
				'error',
				"Missing required parameters 'commandType' or 'serverName'"
			);
			const errorMsg = `
			<server_tool_response>
				...
				<error_details>
					...
					<help>
						<example_usage>
						<action>${serverRunnerPrompt.examples[0].output}</action>
						</example_usage>
		...
		if (
			(commandType === 'start' || commandType === 'restart') &&
			!commandToRun
		) {
			await say(
				'error',
				"Missing required parameter 'commandToRun' for start/restart operation"
			);
			const errorMsg = `
			<server_tool_response>
				<status>
		...
		try {
			...
			TerminalRegistry.updateDevServerStatus(
				devServer?.terminalInfo.id || -1,
				'error',
	...
	private analyzeError(
		errorMessage: string,
		logs: string[]
	): { message: string; troubleshooting: string } {
		...
		if (
			...
			/command not found/i.test(lastLogs)
		) {
			return {
		...
		if (
			/failed to compile/i.test(errorMessage) ||
			/failed to compile/i.test(lastLogs)
		) {
			return {
	...
	private processServerOutput(
		...
		if (
			!startData.serverReady &&
			DevServerTool.SERVER_READY_PATTERNS.some((pattern) => pattern.test(line))
		) {
			startData.serverReady = true;
		}
