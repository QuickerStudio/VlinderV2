 ...
 */
export async function formatAttachementsIntoBlocks(
	resources?: Resource[]
): Promise<string> {
	...
	for (const resource of resources || []) {
		if (resource.type === 'file') {
			...
			additionalContextFiles.push(
				`<file path="${resource.id}">${fileContent}</file>`
			);
		} else if (resource.type === 'url') {
			additionalScrappedUrls.push(
				`<url link="${resource.name}" description="${resource.description}"/>`
			);
		}
	}
