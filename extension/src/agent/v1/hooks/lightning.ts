...
export async function askLightning(
	ctx: ExtensionContext,
	input: AskLightningInput
): Promise<AskLightningResult> {
	try {
		...
		if (!mainAgent) {
			console.log(
				'[Lightning] Main agent not available, initializing with no task'
			);
		...
		try {
			...
			console.log('[Lightning] Using observer model:', {
				modelId: model.id,
				provider: model.provider,
			});
		} catch (_error) {
			console.log(
				'[Lightning] Observer model not configured, falling back to current main model'
			);
			...
			console.log('[Lightning] Using current main model:', {
				modelId: model.id,
				provider: model.provider,
			});
		}
		...
		const targetModel = input.modelId
			? providerData.models.find((m: any) => m.id === input.modelId) || model
			: model;
		...
		for await (const message of response) {
			...
			if ((message as any).code === 1) {
				output =
					(message as any).body.anthropic.content[0].type === 'text'
						? (message as any).body.anthropic.content[0].text
						: '';
				break;
			}
		}
