...
export interface HookOptions {
	...
	/**
	 * Whether the hook should auto-trigger based on its own logic
	 * If true, the hook's shouldExecute() method controls when it triggers
	 * If false or undefined, only triggerEvery controls triggering
	 */
	autoTrigger?: boolean;
}
...
export abstract class BaseHook {
	...
	public shouldTrigger(): boolean {
		...
		if (
			this.hookState.requestsSinceLastTrigger >= this._hookOptions.triggerEvery
		) {
