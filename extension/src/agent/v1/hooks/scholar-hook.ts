...
export class ScholarHook extends BaseHook {
	 ...
	 */
	async requestScholarService(type: 'extract' | 'search' | 'summarize', context?: string): Promise<string | null> {
		...
		try {
			const { providerData, model } = await serverRPC().getClient().currentScholarModel({})
			const { scholarSettings } = await serverRPC().getClient().getScholarSettings({})
			if (!scholarSettings) {
			...
			const taskHistory = [...currentContext.history].slice(-scholarSettings.scholarPullMessages * 2)
			...
			taskHistory.push({
				...
				content: [
					{
						type: "text",
						text: this.createScholarPrompt(type, context || currentContext.taskMsgText),
					},
				],
			})
			...
			const scholarResponse = await apiManager.createApiStreamRequest(
				...
				{
					systemPrompt: scholarSettings.scholarPrompt ?? scholarHookDefaultPrompt,
				},
				true
			)
	...
	private async shouldExecute(): Promise<boolean> {
		try {
			...
			const claudeMessages = this.MainAgent.getStateManager().state.claudeMessages
			...
			const hasLearningOpportunity = await this.detectLearningOpportunity(history.slice(-3))
			const lastMessage = history.slice(-1)?.[0]?.content?.[0]
			const lastMessageText = typeof lastMessage === "string" ? lastMessage : lastMessage?.type === "text" ? lastMessage.text : ""
			const hasExplicitRequest = this.detectExplicitScholarRequest(lastMessageText)
	...
	private detectExplicitScholarRequest(messageText: string): boolean {
		const scholarKeywords = [
			"summarize", "remember", "save knowledge", "extract learning",
			"document this", "store experience", "knowledge base", "memory",
			"what did we learn", "search knowledge", "recall pattern"
		]
	...
	private async detectLearningOpportunity(messages: any[]): Promise<boolean> {
		try {
			...
			console.error("[ScholarHook] Error detecting learning opportunity:", error)
	...
	protected async executeHook(): Promise<string | null> {
		...
		try {
			...
			if (lastMessage?.role === "assistant" && Array.isArray(lastMessage.content)) {
				const scholarPrompt = this.createScholarPrompt(requestType, currentContext.taskMsgText)
				taskHistory.push({
					role: "user",
					content: [
						{
			...
			const responseRequestType = this.determineRequestType(currentContext.history)
			let success = true
			
			if (responseRequestType === 'extract' || responseRequestType === 'summarize') {
			...
			if (finalOutput.length > 0) {
				const statusMessage = this.createStatusMessage(responseRequestType, success, finalOutput)
	 ...
	 */
	private determineRequestType(history: any[]): 'extract' | 'search' | 'summarize' {
		...
		if (messageText.includes("search") || messageText.includes("find") || 
			messageText.includes("recall") || messageText.includes("what did we learn")) {
			return 'search'
		}

		// Check for summarization requests
		if (messageText.includes("summarize") || messageText.includes("remember") || 
			messageText.includes("save this") || messageText.includes("document")) {
			return 'summarize'
		}
	 ...
	 */
	private createScholarPrompt(requestType: 'extract' | 'search' | 'summarize', taskContext: string): string {
		...
		switch (requestType) {
			case 'search':
				return baseContext + dedent`search the knowledge base and provide relevant patterns, solutions, or insights that could help with the current development work.
			...
			case 'summarize':
				return baseContext + dedent`summarize and document the key knowledge from this development session for future reference.
			...
			case 'extract':
			default:
				return baseContext + dedent`analyze the development activity and extract valuable knowledge that should be preserved for future reference.
	 ...
	 */
	private createStatusMessage(requestType: 'extract' | 'search' | 'summarize', success: boolean, output: string): string {
		switch (requestType) {
			case 'search':
	 ...
	 */
	private async updateLearningKeywords(extractedKnowledge: string): Promise<void> {
		try {
			// Extract potential keywords from the knowledge
			const potentialKeywords = this.extractKeywordsFromKnowledge(extractedKnowledge)
			...
			if (potentialKeywords.length > 0) {
				...
				const newKeywords = potentialKeywords.filter(keyword =>
					!existingKeywordsList.includes(keyword.toLowerCase())
				)
				...
				if (newKeywords.length > 0) {
					...
					await serverRPC().getClient().saveLearningKeywords({ keywords: updatedKeywords })

					console.log(`[ScholarHook] Added new learning keywords: ${newKeywords.join(', ')}`)
				}
			}
		} catch (error) {
	 ...
	 */
	private async saveKnowledgeToFile(extractedKnowledge: string, context: any): Promise<boolean> {
		try {
			const cwd = getCwd()
