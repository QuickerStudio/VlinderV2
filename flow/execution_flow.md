...
2. ClaudeDev is instantiated with necessary options.
...
- If it's a new task:
  1. ClaudeDev calls TaskExecutor to start the new task.
  2. The initial task details are added to the state.

- If it's a history item:
  1. ClaudeDev calls a method to resume the task from history.
...
1. Once TaskExecutor determines the task is complete, it notifies ClaudeDev.
2. MainAgent prepares the final result.
3. The result is presented to the user.

## 7. Feedback and Iteration (Optional)

If the user provides feedback:
1. MainAgent receives the feedback.
